...
class _ActualVsPlannedGapAnalysisScreenState
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            dropdownItems: LaunchGapItem.gapStatuses,
            hint: 'Status')
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'planned',
            label: 'Planned',
            sampleValue: 'User portal deployment'),
        CsvColumnSpec(
            key: 'actual',
            label: 'Actual',
            sampleValue: 'Deployed with minor gaps'),
        CsvColumnSpec(
            key: 'gap', label: 'Gap', sampleValue: 'Missing SSO integration'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Partial',
            allowedValues: ['Met', 'Partial', 'Missed', 'Exceeded']),
      ],
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['On Track', 'Delayed', 'Missed', 'Early'],
            hint: 'Status')
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'milestone', label: 'Milestone', sampleValue: 'Go-live'),
        CsvColumnSpec(
            key: 'planned', label: 'Planned', sampleValue: '2025-01-15'),
        CsvColumnSpec(
            key: 'actual', label: 'Actual', sampleValue: '2025-01-18'),
        CsvColumnSpec(
            key: 'variance', label: 'Variance', sampleValue: '+3 days'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'On Track',
            allowedValues: ['On Track', 'Delayed', 'Missed', 'Early']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            hint: 'Variance'),
        LaunchColumn(
            label: '%', width: 110, fieldType: LaunchFieldType.text, hint: '%')
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'category', label: 'Category', sampleValue: 'Infrastructure'),
        CsvColumnSpec(
            key: 'planned', label: 'Planned', sampleValue: '\$100,000'),
        CsvColumnSpec(key: 'actual', label: 'Actual', sampleValue: '\$115,000'),
        CsvColumnSpec(
            key: 'variance', label: 'Variance', sampleValue: '-\$15,000'),
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Open', 'In Progress', 'Resolved'],
            hint: 'Status')
      ],
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'rootCause', label: 'Root Cause', sampleValue: 'Scope creep'),
        CsvColumnSpec(key: 'impact', label: 'Impact', sampleValue: 'High'),
        CsvColumnSpec(
            key: 'action', label: 'Action', sampleValue: 'Rebase budget'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Open',
            allowedValues: ['Open', 'In Progress', 'Resolved']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'action',
            label: 'Action',
            sampleValue: 'Monitor SLA compliance'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Track vendor SLA adherence'),
  ...
  Future<void> _regenerateScopeGapRow(int index) async {
    ...
    try {
      ...
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Scope Gap Analysis');
        ...
        if (mounted) {
          setState(() {
            _scopeGaps[index] = _scopeGaps[index].copyWith(
              ...
              gapDescription:
                  (parsed['gap'] ?? _scopeGaps[index].gapDescription)
                      .toString(),
              gapStatus:
                  (parsed['status'] ?? _scopeGaps[index].gapStatus).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _regenerateMilestoneVarianceRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Milestone Variance');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _milestoneVariances[index] = _milestoneVariances[index].copyWith(
              ...
              varianceDays: (parsed['variance'] ??
                      _milestoneVariances[index].varianceDays)
                  .toString(),
              status: (parsed['status'] ?? _milestoneVariances[index].status)
                  .toString(),
            );
          });
          _save();
        ...
        if (mounted) {
          setState(() {
            _budgetVariances[index] = _budgetVariances[index].copyWith(
              ...
              plannedAmount:
                  (parsed['planned'] ?? _budgetVariances[index].plannedAmount)
                      .toString(),
              actualAmount:
                  (parsed['actual'] ?? _budgetVariances[index].actualAmount)
                      .toString(),
              variance: (parsed['variance'] ?? _budgetVariances[index].variance)
                  .toString(),
              variancePercent:
                  (parsed['percent'] ?? _budgetVariances[index].variancePercent)
                      .toString(),
            );
          });
          _save();
        ...
        if (mounted) {
          setState(() {
            _rootCauses[index] = _rootCauses[index].copyWith(
              ...
              rootCause: (parsed['root_cause'] ?? _rootCauses[index].rootCause)
                  .toString(),
              impact:
                  (parsed['impact'] ?? _rootCauses[index].impact).toString(),
              correctiveAction:
                  (parsed['action'] ?? _rootCauses[index].correctiveAction)
                      .toString(),
              status:
                  (parsed['status'] ?? _rootCauses[index].status).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _regenerateFollowUpRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Follow-Up Actions');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _followUpActions[index] = _followUpActions[index].copyWith(
              ...
              details: (parsed['details'] ?? _followUpActions[index].details)
                  .toString(),
              owner:
                  (parsed['owner'] ?? _followUpActions[index].owner).toString(),
              status: (parsed['status'] ?? _followUpActions[index].status)
                  .toString(),
            );
          });
          _save();
  ...
  Future<void> _autoPopulateFromPriorPhases() async {
    ...
    try {
      ...
      for (final st in cp.scopeTracking) {
        if (st.deliverable.isNotEmpty &&
            !scopeGapExisting.contains(st.deliverable)) {
      ...
      final milestoneExisting =
          _milestoneVariances.map((m) => m.milestone).toSet();
      ...
      for (final sp in cp.planningSprints) {
        ...
        if (title.isNotEmpty && !milestoneExisting.contains(title)) {
          newMilestones.add(LaunchMilestoneVariance(
            milestone: title,
            plannedDate: sp['startDate']?.toString() ??
                sp['plannedDate']?.toString() ??
                '',
            actualDate:
                sp['endDate']?.toString() ?? sp['actualDate']?.toString() ?? '',
            status: 'On Track',
          ));
        }
      }
      ...
      for (final br in cp.budgetRows) {
        ...
        if (cat.isNotEmpty && !budgetExisting.contains(cat)) {
          ...
          final plannedNum =
              double.tryParse(planned.replaceAll(RegExp(r'[^\d.]'), '')) ?? 0;
          final actualNum =
              double.tryParse(actual.replaceAll(RegExp(r'[^\d.]'), '')) ?? 0;
          final variance = plannedNum - actualNum;
          final variancePct = plannedNum > 0
              ? ((variance / plannedNum) * 100).toStringAsFixed(1)
              : '';
      ...
      for (final ri in cp.openRiskItems) {
        ...
        if (title.isNotEmpty && !rootCauseExisting.contains(title)) {
          newRootCauses.add(LaunchRootCauseItem(
            gap: title,
            rootCause:
                ri['cause']?.toString() ?? ri['rootCause']?.toString() ?? '',
      ...
      for (final ri in cp.openRiskItems) {
        ...
        if (title.isNotEmpty && !followUpExisting.contains(title)) {
          newFollowUps.add(LaunchFollowUpItem(
            title: 'Monitor: $title',
            details: ri['description']?.toString() ??
                ri['details']?.toString() ??
                '',
      ...
      for (final mp in cp.mitigationPlans) {
        ...
        if (title.isNotEmpty && !followUpExisting.contains(title)) {
          newFollowUps.add(LaunchFollowUpItem(
            title: title,
            details: mp['description']?.toString() ??
                mp['details']?.toString() ??
                '',
      ...
      if (newScopeGaps.isNotEmpty ||
          ...
          newBudgetVariances.isNotEmpty ||
          newRootCauses.isNotEmpty ||
  ...
  Future<void> _populateFromAi() async {
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          'scope_gaps':
              'Scope gap items with "planned", "actual", "gap_description", "gap_status"',
          'milestone_variances':
              'Milestone variances with "milestone", "planned_date", "actual_date", "variance_days", "status"',
          'budget_variances':
              'Budget variance items with "category", "planned_amount", "actual_amount", "variance", "variance_percent"',
          'root_causes':
              'Root cause items with "gap", "root_cause", "impact", "corrective_action", "status"',
          'follow_up_actions':
              'Follow-up actions with "title", "details", "owner", "status"',
        },
        itemsPerSection: 3,
      );
    } catch (e) {
          ...
          build: (_) => [
            ...
            if (_followUpActions.isEmpty)
              ...
              pw.TableHelper.fromTextArray(
                ...
                cellPadding:
                    const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 5),
                headers: const ['Action', 'Details', 'Owner', 'Status'],
  ...
  Widget _buildLaunchInsights() {
    final projectData = ProjectDataHelper.getData(context);
    final scopeTotal =
        projectData.withinScope.where((s) => s.trim().isNotEmpty).length +
            projectData.outOfScope.where((s) => s.trim().isNotEmpty).length;
    final completionPct = scopeTotal == 0
        ? 0.0
        : (projectData.withinScope.where((s) => s.trim().isNotEmpty).length /
            scopeTotal);
    return LaunchInsightsHeader(
      ...
      kpiTiles: [
        ...
        LaunchKpiTile(
          ...
          value:
              '\$${projectData.frontEndPlanning.allowanceItems.fold<double>(0, (s, i) => s + i.actualAmount).toStringAsFixed(0)}',
