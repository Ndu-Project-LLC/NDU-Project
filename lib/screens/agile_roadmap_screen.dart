...
class _AgileRoadmapScreenState extends State<AgileRoadmapScreen>
    ...
    try {
      ...
      try {
        final epicsSnapshot = await FirebaseFirestore.instance
            ...
            .get();
        _epics =
            epicsSnapshot.docs.map((d) => {'id': d.id, ...d.data()}).toList();
      } catch (_) {}
  ...
  Widget _hierarchyLevel(
      String levelLabel, String title, Color color, IconData icon) {
    return Container(
          ...
          child: Column(
            children: [
              ...
              Wrap(
                ...
                children: [
                  _progressStat('Sprints',
                      '${_sprints.where((s) => (s['status'] ?? '').toLowerCase() == 'completed').length}/${_sprints.length}'),
  ...
  Widget _buildMilestoneManagement() {
    return Column(
      ...
      children: [
        _sectionTitle(
            'Milestone Management', 'Track major delivery objectives'),
        const SizedBox(height: 20),
        Container(
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(
          ...
          child: _milestones.isEmpty
              ? const Center(
                  child: Padding(
                    padding: EdgeInsets.all(20),
                    child: Text(
                        'No milestones yet. Data flows from project setup.',
                        style:
  ...
  Widget _milestoneCard(Map<String, dynamic> m) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border:
            Border.all(color: const Color(0xFFFCD34D).withValues(alpha: 0.3)),
      ),
      child: Row(
        children: [
          Container(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  _depType(
                      'Critical Path', Icons.timeline, const Color(0xFFEF4444)),
                ],
              ),
            ],
          ),
