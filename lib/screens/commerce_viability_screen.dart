...
class _CommerceViabilityScreenState extends State<CommerceViabilityScreen> {
  ...
  Widget _buildFinancialMetricsPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'notes', label: 'Notes', sampleValue: '3-year projection'),
      ],
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            dropdownItems: ['Active', 'Expiring Soon', 'Expired', 'Claimed']),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'item', label: 'Item', sampleValue: 'Server hardware'),
        ...
        CsvColumnSpec(
            key: 'expiry', label: 'Expiry', sampleValue: '2028-01-01'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Active',
            allowedValues: ['Active', 'Expiring Soon', 'Expired', 'Claimed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'recommendation',
            label: 'Recommendation',
            sampleValue: 'Negotiate extended warranty'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Extend server warranty by 2 years'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Open',
  ...
  Future<void> _regenerateFinancialMetricRow(int index) async {
    ...
    try {
      ...
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Financial Metrics');
        ...
        if (mounted) {
          setState(() {
            _financialMetrics[index] = _financialMetrics[index].copyWith(
              ...
              notes: (parsed['notes'] ?? _financialMetrics[index].notes)
                  .toString(),
            );
          });
          _save();
        ...
        if (mounted) {
          setState(() {
            _warranties[index] = _warranties[index].copyWith(
              ...
              vendor:
                  (parsed['vendor'] ?? _warranties[index].vendor).toString(),
              warrantyType: (parsed['type'] ?? _warranties[index].warrantyType)
                  .toString(),
              status:
                  (parsed['status'] ?? _warranties[index].status).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _regenerateOpsCostRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Operations Cost Projection');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _opsCosts[index] = _opsCosts[index].copyWith(
              ...
              monthlyCost: (parsed['monthly'] ?? _opsCosts[index].monthlyCost)
                  .toString(),
              annualCost:
                  (parsed['annual'] ?? _opsCosts[index].annualCost).toString(),
        ...
        if (mounted) {
          setState(() {
            _recommendations[index] = _recommendations[index].copyWith(
              ...
              details: (parsed['details'] ?? _recommendations[index].details)
                  .toString(),
              status: (parsed['status'] ?? _recommendations[index].status)
                  .toString(),
            );
          });
          _save();
    ...
    try {
      ...
      for (final c in cp.contracts) {
        if (c.contractName.isNotEmpty &&
            !warrantyExisting.contains(c.contractName)) {
      ...
      if (cp.totalPlannedBudget > 0 &&
          !metricExisting.contains('Total Planned Budget')) {
      ...
      if (cp.totalActualBudget > 0 &&
          !metricExisting.contains('Total Actual Spend')) {
        newMetrics.add(LaunchFinancialMetric(
      ...
      if (cp.totalContractValue > 0 &&
          !metricExisting.contains('Total Contract Value')) {
        newMetrics.add(LaunchFinancialMetric(
      ...
      if (newWarranties.isNotEmpty ||
          newOpsCosts.isNotEmpty ||
          newMetrics.isNotEmpty ||
          newRecs.isNotEmpty) {
        await _persistData();
      }
    } catch (e) {
  ...
  Future<void> _populateFromAi() async {
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          'financial_metrics': 'ROI metrics with "label", "value", "notes"',
          'warranties':
          ...
          'recommendations':
              'Recommendations with "title", "details", "status"',
        },
        itemsPerSection: 3,
      );
    } catch (e) {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final filename =
          'commerce_viability_${projectName.replaceAll(' ', '_')}_$stamp.pdf';
  ...
  Widget _buildLaunchInsights() {
    ...
    final completionPct = totalProviders == 0
        ? 0.0
        : (projectData.vendors.length / totalProviders).clamp(0.0, 1.0);
    return LaunchInsightsHeader(
      sectionTitle: 'Hypercare & Warranty Status',
      sectionSubtitle:
          'Warranty coverage, hypercare tickets, and SLA performance',
      ...
      kpiTiles: [
        ...
        LaunchKpiTile(
          label: 'Open Risks',
          value:
              '${projectData.frontEndPlanning.riskRegisterItems.where((r) => r.status.toLowerCase() != 'closed').length}',
