...
class ContractsTrackingScreen extends StatefulWidget {
  ...
  static void open(BuildContext context) {
    context.push('/contracts-tracking');
  }
...
class _ContractsTrackingScreenState extends State<ContractsTrackingScreen> {
  ...
  List<_RenewalLaneData> _defaultRenewalLanes() {
    ...
    return [
      _RenewalLaneData(
        ...
        expiryDate:
            DateFormat('MMM d, yyyy').format(now.add(const Duration(days: 18))),
        daysUntilExpiry: 18,
        ...
        committedValue: '\$2.4M',
        notes:
            'SLA uptime requirement increasing to 99.95%; bandwidth upgrade needed',
      ),
      _RenewalLaneData(
        id: _newId(),
        ...
        expiryDate:
            DateFormat('MMM d, yyyy').format(now.add(const Duration(days: 42))),
        daysUntilExpiry: 42,
        renewalAction: 'Renew',
        owner: 'PMO Lead',
        status: 'Draft Ready',
      ...
      _RenewalLaneData(
        ...
        contractType: 'Service Level Agreement',
        expiryDate:
            DateFormat('MMM d, yyyy').format(now.add(const Duration(days: 67))),
        daysUntilExpiry: 67,
        renewalAction: 'Renew',
        ...
        committedValue: '\$560K',
        notes:
            'Annual physical security review required before renewal approval',
      ),
      _RenewalLaneData(
        id: _newId(),
        ...
        expiryDate:
            DateFormat('MMM d, yyyy').format(now.add(const Duration(days: 89))),
        daysUntilExpiry: 89,
        ...
        committedValue: '\$95K',
        notes:
            'Regulatory audit scope expansion pending; hold renewal until scope confirmed',
      ),
      _RenewalLaneData(
        id: _newId(),
        ...
        expiryDate: DateFormat('MMM d, yyyy')
            .format(now.add(const Duration(days: 125))),
        daysUntilExpiry: 125,
        renewalAction: 'Renegotiate',
        ...
        committedValue: '\$3.1M',
        notes:
            'Market rate review needed; competitor quotes due by next quarter',
      ),
    ];
  }
  ...
  List<_ApprovalCheckpointData> _defaultApprovalCheckpoints() {
    return [
      _ApprovalCheckpointData(
        ...
        description:
            'Verify contract terms against regulatory requirements, liability clauses, and IP provisions',
      ...
      _ApprovalCheckpointData(
        ...
        description:
            'Confirm budget allocation, payment schedule, and fiscal compliance for committed value',
      ...
      _ApprovalCheckpointData(
        ...
        description:
            'Validate that contracted scope matches approved project scope and WBS deliverables',
      ...
      _ApprovalCheckpointData(
        ...
        description:
            'Confirm vendor capability and technical approach meet acceptance criteria and SLA targets',
      ...
      _ApprovalCheckpointData(
        ...
        description:
            'Final approval from executive sponsor for contract execution and vendor onboarding',
      ...
      _ApprovalCheckpointData(
        ...
        description:
            'Confirm vendor insurance certificates, indemnity provisions, and risk transfer mechanisms',
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return RepaintBoundary(
                  key: ValueKey('renewal_row_$index'),
                  child: _RenewalEntryRow(
                    entry: lane,
          ...
          if (_approvalCheckpoints.isEmpty)
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return RepaintBoundary(
                  key: ValueKey('approval_row_$index'),
                  child: _ApprovalGateRow(
                    checkpoint: checkpoint,
                    onEdit: () =>
                        _showApprovalCheckpointEditor(checkpoint: checkpoint),
                    onDelete: () =>
                        _confirmDeleteApprovalCheckpoint(checkpoint),
                    showDivider: !isLast,
                  ),
                );
              },
  ...
  Future<void> _showRenewalEntryEditor({_RenewalLaneData? entry}) async {
    ...
    if (entry?.expiryDate != null && entry!.expiryDate.isNotEmpty) {
      selectedExpiryDate =
          DateFormat('MMM dd, yyyy').tryParse(entry.expiryDate) ??
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: Text(
                  isEdit ? 'Edit renewal entry' : 'Add contract to pipeline'),
              content: SizedBox(
                width: 580,
                    ...
                    children: [
                      ...
                      VoiceTextField(
                        ...
                        decoration: const InputDecoration(
                          labelText: 'Notes',
                          hintText:
                              'Renegotiation context, key terms, or blockers',
                          isDense: true,
                        ),
                        maxLines: 3,
                      ),
                  ...
                  onPressed: () {
                    if (nameController.text.trim().isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text('Contract name is required.')),
                      );
                      return;
                    }
  ...
  Future<void> _showApprovalCheckpointEditor(
    ...
    var selectedPriority =
        _approvalPriorityOptions.contains(checkpoint?.priority)
            ? checkpoint!.priority
    ...
    if (saved == null) return;
    final index =
        _approvalCheckpoints.indexWhere((item) => item.id == saved.id);
    setState(() {
      if (index == -1) {
  ...
  Future<void> _confirmDeleteApprovalCheckpoint(
    ...
    if (confirmed == true) {
      setState(() =>
          _approvalCheckpoints.removeWhere((item) => item.id == checkpoint.id));
      _scheduleSave();
    }
  }
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_contracts_tracking_notes'] ??
                'No data recorded.'),
      ],
    );
  }
  ...
  static List<_ApprovalCheckpointData> fromList(dynamic data) {
    ...
    return data.map((item) {
      ...
      return _ApprovalCheckpointData(
        ...
        targetDate:
            map['targetDate']?.toString() ?? map['dueDate']?.toString() ?? '',
