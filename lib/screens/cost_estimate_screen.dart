...
''',
  ...
  Future<void> _autoPopulateFromInitiationIfNeeded() async {
    ...
    final initialized =
        await _isSectionInitialized('cost_estimate_initialized');
    if (initialized) {
      _autoPopulated = true;
      return;
    }
...
class _BoeSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final item in items) {
      methods[item.estimatingMethod] =
          (methods[item.estimatingMethod] ?? 0) + 1;
      ...
      if (item.designMaturity.isNotEmpty) {
        maturityLevels[item.designMaturity] =
            (maturityLevels[item.designMaturity] ?? 0) + 1;
      }
            ...
            children: [
              ...
              Expanded(
                child: _boeColumn(
                  'Documentation Gap',
                  [
                    _boeGapRow('Estimating Basis', withEstimatingBasis,
                        totalItems, missingBasis),
                    _boeGapRow('Scope In/Excluded', documentedScope, totalItems,
                        missingScope),
                  ],
                ),
              ),
  ...
  Widget _boeGapRow(String label, int documented, int total, int missing) {
    ...
    final color =
        missing == 0 ? const Color(0xFF059669) : const Color(0xFFC2410C);
    return Padding(
      padding: const EdgeInsets.only(bottom: 6),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
                ...
                child: _profileColumn('Agile Delivery', [
                  if (agileCount > 0) ...[
                    ...
                    _profileRow(
                        '% of Total Budget',
                        wpTotalBudget > 0
                            ? '${(agileCost / wpTotalBudget * 100).toStringAsFixed(1)}%'
                            : '0%'),
                  ] else
...
class _ContractStrategyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalContractValue =
        contracts.fold<double>(0, (s, c) => s + c.estimatedCost);
    final totalCostValue =
        costItems.fold<double>(0, (s, item) => s + item.amount);
    final pctLinkedToContracts =
        totalCostValue > 0 ? (linkedCost / totalCostValue * 100) : 0.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _strategyStat('Total Contract Value',
              ...
              _strategyStat(
                  'Coverage',
                  '${pctLinkedToContracts.toStringAsFixed(1)}% of total',
                  pctLinkedToContracts > 50
          ...
          ...ContractStatus.values
              .where((s) => (byStatusCount[s] ?? 0) > 0)
              .map((status) {
            final value = byStatus[status] ?? 0;
            final count = byStatusCount[status] ?? 0;
            final pct =
                totalContractValue > 0 ? value / totalContractValue : 0.0;
            return Padding(
              padding: const EdgeInsets.only(bottom: 6),
              child: Row(
                children: [
      ...
      children: [
        ...
        ListView.builder(
          ...
          itemBuilder: (context, i) => Padding(
            ...
            child: _SourceSummaryCard(summary: sourceSummaries[i]),
          ),
        ),
      ],
    );
...
class _ContingencyRiskPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final pertItems = forecastItems
        ...
        .toList();
    final pertBaseTotal =
        pertItems.fold<double>(0, (total, item) => total + item.amount);
    final pertMeanTotal =
        pertItems.fold<double>(0, (total, item) => total + item.pertMean);
    final pertExposureTotal =
        pertItems.fold<double>(0, (total, item) => total + item.pertExposure);
    final reserve = projectData.managementReserve;
    final forecastTotal =
        forecastItems.fold<double>(0, (total, item) => total + item.amount);
  ...
  Widget _riskDomainTile(String label, int count, double total,
      double forecastTotal, IconData icon, String description, Color color) {
              ...
              children: [
                ...
                if (node.designMaturity.isNotEmpty)
                  Padding(
                    ...
                    child: _DesignMaturityBadge(
                        designMaturity: node.designMaturity),
                  ),
                SizedBox(
                    width: 100,
                    child: Text(formatCurrency(node.planned),
                        style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                ...
                SizedBox(
                    width: 100,
                    child: Text(
                        node.actual > 0 ? formatCurrency(node.actual) : '-',
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                            color: node.actual > 0
...
class _CostVsScheduleWorkspace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final wp in workPackages) {
      final start =
          _tryParseDate(wp.plannedStart) ?? _tryParseDate(wp.actualStart);
      ...
      final monthlyActualAmount =
          wp.actualCost > 0 ? wp.actualCost / months : 0.0;

      var cursor = DateTime(start.year, start.month, 1);
      while (!cursor.isAfter(end)) {
        final key = _monthKey(cursor);
        monthlyPlanned.update(key, (v) => v + monthlyPlannedAmount,
            ifAbsent: () => monthlyPlannedAmount);
        if (monthlyActualAmount > 0) {
          monthlyActual.update(key, (v) => v + monthlyActualAmount,
              ifAbsent: () => monthlyActualAmount);
        }
    ...
    for (final key in sortedMonths) {
      ...
      plannedData
          .add(SCurveDataPoint(date: date, cumulativeCost: plannedCumulative));
      actualData
          .add(SCurveDataPoint(date: date, cumulativeCost: actualCumulative));
    }
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          _evmMetric('CV', formatCurrency(cv),
        ...
        children: [
          ...
          if (ratePercent > 0) ...[
            ...
            if (ratePercent > 0) ...[
              ...
              _overheadRow(
                  'Total incl. Overhead', directBaseTotal + overheadAmount,
                  bold: true),
            ],
          ],
          const SizedBox(height: 10),
  ...
  void initState() {
    ...
    _selectedEstimatingMethod =
        existing?.estimatingMethod.trim().isNotEmpty == true
            ...
            : 'manual';
    _selectedDesignMaturity = existing?.designMaturity.trim().isNotEmpty == true
        ? existing!.designMaturity
        : '';
    _selectedRateSource = existing?.rateSource.trim().isNotEmpty == true
        ? existing!.rateSource
        : '';
                            ...
                            Expanded(
                              child: _dropdownField<String>(
                                ...
                                items: const [
                                  ...
                                  DropdownMenuItem(
                                      value: '10%',
                                      child: Text('10% - Concept')),
                                  DropdownMenuItem(
                                      value: '30%',
                                      child: Text('30% - Preliminary')),
                                  DropdownMenuItem(
                                      value: '60%',
                                      child: Text('60% - Detailed')),
                                  DropdownMenuItem(
                                      value: '90%',
                                      child: Text('90% - Pre-IFC')),
                                  DropdownMenuItem(
                                      value: 'IFC', child: Text('IFC')),
                                  DropdownMenuItem(
                                      value: 'AsBuilt',
                                      child: Text('As-Built')),
                                ],
                            ...
                            Expanded(
                              child: _dropdownField<String>(
                                ...
                                items: const [
                                  ...
                                  DropdownMenuItem(
                                      value: 'vendor_quote',
                                      child: Text('Vendor Quote')),
                                  DropdownMenuItem(
                                      value: 'historical',
                                      child: Text('Historical Data')),
                                  DropdownMenuItem(
                                      value: 'published_index',
                                      child: Text('Published Index')),
                                  DropdownMenuItem(
                                      value: 'benchmark',
                                      child: Text('Benchmark')),
                                  DropdownMenuItem(
                                      value: 'expert_judgment',
                                      child: Text('Expert Judgment')),
                                ],
                                onChanged: (value) {
                                  if (value == null) return;
