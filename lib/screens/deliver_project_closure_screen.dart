...
class _DeliverProjectClosureScreenState
  ...
  Widget _buildLaunchInsights() {
    ...
    return LaunchInsightsHeader(
      ...
      sectionSubtitle:
          'Go-live readiness across people, process, technology & operations',
      ...
      kpiTiles: [
        ...
        LaunchKpiTile(
          ...
          delta:
              '${projectData.frontEndPlanning.riskRegisterItems.where((r) => r.status.toLowerCase() != 'closed').length} open',
        ),
        LaunchKpiTile(
          label: 'Allowances',
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            ...
            label: 'Status',
            sampleValue: 'Pending',
            allowedValues: ['Pending', 'In Progress', 'Complete', 'Delayed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Critical UI bug in production'),
        CsvColumnSpec(key: 'owner', label: 'Owner', sampleValue: 'Tech Lead'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Open',
            allowedValues: ['Open', 'In Progress', 'Complete', 'Deferred']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'title',
            label: 'Title',
            sampleValue: 'Performance degradation'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'API latency above threshold'),
      ...
      if (_milestones.isEmpty && cp.planningSprints.isNotEmpty) {
        final newMilestones = cp.planningSprints
            .map((s) => LaunchMilestone(
                  title:
                      'Sprint ${s['sprintNumber'] ?? s['name'] ?? '?'}: ${s['goal'] ?? s['title'] ?? ''}',
      ...
      if (_riskFollowUps.isEmpty && cp.openRiskItems.isNotEmpty) {
        ...
        final newRisks = cp.openRiskItems
            .where((r) => !existing.contains(
                r['title']?.toString() ?? r['risk']?.toString() ?? ''))
  ...
  void _populateWithSeedData() {
    ...
    final twoWeeksStr =
        '${twoWeeks.year}-${twoWeeks.month.toString().padLeft(2, '0')}-${twoWeeks.day.toString().padLeft(2, '0')}';
    ...
    setState(() {
      _scopeItems = [
        LaunchScopeItem(
          ...
          acceptanceCriteria:
              'All user acceptance tests pass; no critical bugs',
          status: 'Pending',
        ...
        LaunchScopeItem(
          ...
          acceptanceCriteria:
              'Automated deploy to staging succeeds; rollback tested',
          status: 'Pending',
        ...
        LaunchScopeItem(
          ...
          acceptanceCriteria:
              'Production data migrated with zero data loss verified',
          status: 'Partial',
          acceptanceDate: twoWeeksStr,
        ),
      ];
      ...
      _outstandingItems = [
        LaunchFollowUpItem(
          ...
          details:
              'Complete load test for 500 concurrent users and document results',
          owner: 'Tech Lead',
          status: 'In Progress',
        ),
        LaunchFollowUpItem(
          ...
          details:
              'Address 3 medium-severity findings from the penetration test',
        ...
        LaunchFollowUpItem(
          ...
          details:
              'Procure production licenses for all third-party integrations',
      ...
      _riskFollowUps = [
        LaunchFollowUpItem(
          ...
          details:
              'Monitor usage against vendor API quotas during initial launch period',
        ...
        LaunchFollowUpItem(
          ...
          details:
              'Ensure database snapshots are taken before go-live for rollback capability',
          owner: 'DBA',
          status: 'Open',
        ),
        LaunchFollowUpItem(
          ...
          details:
              'Confirm key stakeholders are available for go-live support window',
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          ...
          'outstanding':
              'Outstanding items with "title", "details", "owner", "status"',
          'risk_followups':
              'Post-delivery risks with "title", "details", "owner", "status"',
        },
        itemsPerSection: 3,
      );
    } catch (e) {
        ...
        if (mounted) {
          setState(() {
            _milestones[index] = _milestones[index].copyWith(
              ...
              status:
                  (parsed['status'] ?? _milestones[index].status).toString(),
            );
          });
          _scheduleSave();
        }
        ...
        if (mounted) {
          setState(() {
            _outstandingItems[index] = _outstandingItems[index].copyWith(
              ...
              owner: (parsed['owner'] ?? _outstandingItems[index].owner)
                  .toString(),
              status: (parsed['status'] ?? _outstandingItems[index].status)
                  .toString(),
            );
          });
          _scheduleSave();
        ...
        if (mounted) {
          setState(() {
            _riskFollowUps[index] = _riskFollowUps[index].copyWith(
              ...
              details: (parsed['details'] ?? '').toString(),
              owner:
                  (parsed['owner'] ?? _riskFollowUps[index].owner).toString(),
              status:
                  (parsed['status'] ?? _riskFollowUps[index].status).toString(),
            );
          });
          _scheduleSave();
        }
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final projectName =
          projectData.projectName.isEmpty ? 'Project' : projectData.projectName;
      final now = DateTime.now();
      final stamp =
...
class _ScopeEditDialogState extends State<_ScopeEditDialog> {
  ...
  Future<void> _generateFieldWithAi(
  ...
  InputDecoration _inputDecoration(
      String hint, TextEditingController controller, String field) {
            ...
            child: Column(
              ...
              children: [
                ...
                VoiceTextField(
                  ...
                  style:
                      const TextStyle(fontSize: 13, color: Color(0xFF1A1D1F)),
                ...
                VoiceTextField(
                  ...
                  style:
                      const TextStyle(fontSize: 13, color: Color(0xFF1A1D1F)),
                  decoration: _inputDecoration(
                      'Acceptance criteria', _criteriaCtrl, 'Criteria'),
                  onChanged: (_) => setState(() {}),
                ),
                const SizedBox(height: 14),
