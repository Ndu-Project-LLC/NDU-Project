...
class _DemobilizeTeamScreenState extends State<DemobilizeTeamScreen> {
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            dropdownItems: ['Active', 'Transitioning', 'Released']),
      ],
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'contact', label: 'Contact', sampleValue: 'john@example.com'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Active',
            allowedValues: ['Active', 'Transitioning', 'Released']),
      ],
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Pending', 'Scheduled', 'Complete']),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'topic', label: 'Topic', sampleValue: 'API Documentation'),
        ...
        CsvColumnSpec(
            ...
            label: 'Status',
            sampleValue: 'Pending',
            allowedValues: ['Pending', 'Scheduled', 'Complete']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
  ...
  Widget _buildVendorOffboardingPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'task', label: 'Task', sampleValue: 'Revoke system access'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Remove VPN and email'),
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Planned', 'Sent', 'Cancelled']),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'audience',
            label: 'Audience',
            sampleValue: 'All Team Members'),
        CsvColumnSpec(
            key: 'message',
            label: 'Message',
            sampleValue: 'Project wrap-up notification'),
        CsvColumnSpec(key: 'channel', label: 'Channel', sampleValue: 'Email'),
        CsvColumnSpec(
            key: 'sendDate', label: 'Send Date', sampleValue: '2025-01-20'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Planned',
            allowedValues: ['Planned', 'Sent', 'Cancelled']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
  ...
  Future<void> _regenerateTeamRosterRow(int index) async {
    ...
    try {
      ...
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Team Ramp-Down Roster');
        ...
        if (mounted) {
          setState(() {
            _teamRoster[index] = _teamRoster[index].copyWith(
              ...
              contact:
                  (parsed['contact'] ?? _teamRoster[index].contact).toString(),
              releaseStatus:
                  (parsed['status'] ?? _teamRoster[index].releaseStatus)
                      .toString(),
            );
          });
          _save();
  ...
  Future<void> _regenerateKnowledgeTransferRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Knowledge Transfer');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _knowledgeTransfers[index] = _knowledgeTransfers[index].copyWith(
              ...
              fromPerson:
                  (parsed['from'] ?? _knowledgeTransfers[index].fromPerson)
                      .toString(),
              toPerson: (parsed['to'] ?? _knowledgeTransfers[index].toPerson)
                  .toString(),
              method: (parsed['method'] ?? _knowledgeTransfers[index].method)
                  .toString(),
              status: (parsed['status'] ?? _knowledgeTransfers[index].status)
                  .toString(),
            );
          });
          _save();
  ...
  Future<void> _regenerateVendorOffboardingRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Vendor Offboarding');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _vendorOffboarding[index] = _vendorOffboarding[index].copyWith(
              ...
              details: (parsed['details'] ?? _vendorOffboarding[index].details)
                  .toString(),
              owner: (parsed['owner'] ?? _vendorOffboarding[index].owner)
                  .toString(),
              status: (parsed['status'] ?? _vendorOffboarding[index].status)
                  .toString(),
            );
          });
          _save();
  ...
  Future<void> _regenerateCommunicationRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Communications & People Care');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _communications[index] = _communications[index].copyWith(
              ...
              message: (parsed['message'] ?? _communications[index].message)
                  .toString(),
              channel: (parsed['channel'] ?? _communications[index].channel)
                  .toString(),
              status: (parsed['status'] ?? _communications[index].status)
                  .toString(),
            );
          });
          _save();
    ...
    try {
      ...
      for (final v in cp.vendors) {
        if (v.vendorName.isNotEmpty &&
            !vendorExisting.contains('Offboard: ${v.vendorName}')) {
      ...
      if (cp.staffing.isNotEmpty &&
          !commsExisting.contains('All Team Members')) {
        newComms.add(LaunchCommunicationItem(
      ...
      if (_teamRoster.isNotEmpty ||
          newKts.isNotEmpty ||
          newVendors.isNotEmpty ||
          newComms.isNotEmpty) {
        await _persistData();
      }
    } catch (e) {
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          ...
          'vendor_offboarding':
              'Vendor offboarding tasks with "title", "details", "status"',
          'communications':
  ...
  Widget _buildLaunchInsights() {
    ...
    final doneMs = projectData.keyMilestones
        ...
        .length;
    final completionPct =
        totalMs == 0 ? (totalPeople == 0 ? 0.0 : 0.1) : doneMs / totalMs;
    return LaunchInsightsHeader(
      sectionTitle: 'Team Demobilization Progress',
      sectionSubtitle:
          'Offboarding, reassignments, and operations/production transition',
