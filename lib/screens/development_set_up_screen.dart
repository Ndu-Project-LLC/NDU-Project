...
class _DevelopmentSetUpScreenState extends State<DevelopmentSetUpScreen> {
  ...
  List<_ApprovalGateItem> _defaultApprovalGates() {
    return [
      _ApprovalGateItem(
        ...
        description:
            'All environments provisioned, tooling licensed, team onboarded',
        status: 'In Progress',
        approver: 'Dev Lead',
        targetDate: 'TBD',
      ),
      ...
      _ApprovalGateItem(
        ...
        description:
            'CI/CD pipeline operational, staging accessible, smoke tests passing',
        status: 'In Progress',
      ...
      _ApprovalGateItem(
        ...
        description:
            'DoR/DoD established, quality gates configured, test coverage baseline set',
        status: 'Planned',
        approver: 'QA Lead',
        targetDate: 'TBD',
      ),
      _ApprovalGateItem(
        ...
        description:
            'Final approval from executive sponsor for development to commence',
        status: 'Not Started',
        ...
        child: Column(
          ...
          children: [
            ...
            LaunchPhaseNavigation(
              backLabel:
                  PlanningPhaseNavigation.backLabel('development_set_up'),
              nextLabel:
                  PlanningPhaseNavigation.nextLabel('development_set_up'),
              onBack: () => PlanningPhaseNavigation.goToPrevious(
                  context, 'development_set_up'),
          ...
          InkWell(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  const Expanded(
                    child: Text('Development setup best practices',
                        style: TextStyle(
                            fontSize: 16,
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildGuideCard(
                      Icons.dns_outlined,
                      'Environment First',
                      'Provision and validate all environments before onboarding the team. Each environment should mirror its target configuration to prevent late-stage surprises.',
                      const Color(0xFFFFC812)),
                  const SizedBox(height: 12),
                  _buildGuideCard(
                      Icons.play_circle_outline,
                      'Pipeline Automation',
                      'Automate build, test, and deployment from day one. Fast feedback loops catch issues early and reduce manual coordination overhead.',
                      const Color(0xFF10B981)),
                  const SizedBox(height: 12),
                  _buildGuideCard(
                      Icons.handyman_outlined,
                      'License Governance',
                      'Track tool licenses, assigned users, and expiry dates. Expired licenses block the team and create compliance risk.',
                      const Color(0xFFF59E0B)),
                  const SizedBox(height: 12),
                  _buildGuideCard(
                      Icons.shield_outlined,
                      'Shift-Left Security',
                      'Integrate security scanning (SAST/DAST) in the CI pipeline from the start. Security debt compounds rapidly if deferred to later phases.',
  ...
  Widget _buildEnvProvisionRegister() {
    return _PanelShell(
      ...
      subtitle:
          'Track environment provisioning status, access URLs, and ownership across all deployment spaces.',
        ...
        children: [
          ...
          if (_envItems.isEmpty)
            const Padding(
                ...
                child: Center(
                    child: Text(
                        'No environments registered. Click Add to create one.',
                        style: TextStyle(color: Color(0xFF6B7280)))))
          else
            ...List.generate(
                _envItems.length, (i) => _buildEnvRow(_envItems[i], i)),
        ],
      ),
    );
  }
            ...
            child: Row(
              children: [
                IconButton(
                    ...
                    onPressed: () => _openEnvDialog(existing: item),
                ...
                IconButton(
                    ...
                    onPressed: () => _confirmDelete('environment', item.id, () {
                          setState(() =>
                              _envItems.removeWhere((e) => e.id == item.id));
                          _scheduleSave();
                        }),
                    padding: EdgeInsets.zero,
                    constraints:
  ...
  Future<void> _openEnvDialog({_EnvProvisionItem? existing}) async {
    ...
    final envCtrl =
        SpellCheckTextEditingController(text: existing?.environment ?? '');
    final typeCtrl =
        SpellCheckTextEditingController(text: existing?.type ?? '');
    final statusCtrl = SpellCheckTextEditingController(
        text: existing?.status ?? 'Not Started');
    final urlCtrl =
        SpellCheckTextEditingController(text: existing?.accessUrl ?? '');
    final ownerCtrl =
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          _dialogField('Status', statusCtrl,
              hint: 'Provisioned / In Progress / Not Started'),
  ...
  Widget _buildCicdPipelineRegister() {
    return _PanelShell(
      ...
      subtitle:
          'Track pipeline stages, tools, triggers, and gate criteria for the build-test-deploy workflow.',
      trailing: OutlinedButton.icon(
        ...
        children: [
          ...
          if (_cicdItems.isEmpty)
            const Padding(
                ...
                child: Center(
                    child: Text(
                        'No pipeline stages registered. Click Add to create one.',
                        style: TextStyle(color: Color(0xFF6B7280)))))
          else
            ...List.generate(
                _cicdItems.length, (i) => _buildCicdRow(_cicdItems[i], i)),
        ],
      ),
    );
  }
            ...
            child: Row(
              children: [
                IconButton(
                    icon: const Icon(Icons.edit_outlined,
                        size: 16, color: Color(0xFF64748B)),
                    onPressed: () => _openCicdDialog(existing: item),
                    padding: EdgeInsets.zero,
                    constraints:
                        const BoxConstraints(minWidth: 28, minHeight: 28)),
                IconButton(
                    icon: const Icon(Icons.delete_outline,
                        size: 16, color: Color(0xFFEF4444)),
                    onPressed: () =>
                        _confirmDelete('pipeline stage', item.id, () {
                          setState(() =>
                              _cicdItems.removeWhere((e) => e.id == item.id));
                          _scheduleSave();
                        }),
                    padding: EdgeInsets.zero,
                    constraints:
  ...
  Future<void> _openCicdDialog({_CicdPipelineItem? existing}) async {
    final isEdit = existing != null;
    final stageCtrl =
        SpellCheckTextEditingController(text: existing?.stage ?? '');
    final toolCtrl =
        SpellCheckTextEditingController(text: existing?.tool ?? '');
    final statusCtrl =
        SpellCheckTextEditingController(text: existing?.status ?? 'Pending');
    final triggerCtrl =
        SpellCheckTextEditingController(text: existing?.trigger ?? '');
    final gateCtrl =
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          _dialogField('Status', statusCtrl,
              hint: 'Ready / Pending / Not Started'),
  ...
  Widget _buildDevToolsRegister() {
    return _PanelShell(
      ...
      subtitle:
          'Track tool licensing, assigned users, status, and expiry dates for the project toolchain.',
      trailing: OutlinedButton.icon(
        ...
        children: [
          ...
          if (_toolItems.isEmpty)
            const Padding(
                padding: EdgeInsets.all(32),
                child: Center(
                    child: Text('No tools registered. Click Add to create one.',
                        style: TextStyle(color: Color(0xFF6B7280)))))
          else
            ...List.generate(
                _toolItems.length, (i) => _buildToolRow(_toolItems[i], i)),
        ],
      ),
    );
  }
            ...
            child: Row(
              children: [
                IconButton(
                    icon: const Icon(Icons.edit_outlined,
                        size: 16, color: Color(0xFF64748B)),
                    onPressed: () => _openToolDialog(existing: item),
                    padding: EdgeInsets.zero,
                    constraints:
                        const BoxConstraints(minWidth: 28, minHeight: 28)),
                IconButton(
                    icon: const Icon(Icons.delete_outline,
                        size: 16, color: Color(0xFFEF4444)),
                    onPressed: () => _confirmDelete('tool', item.id, () {
                          setState(() =>
                              _toolItems.removeWhere((e) => e.id == item.id));
                          _scheduleSave();
                        }),
                    padding: EdgeInsets.zero,
                    constraints:
  ...
  Future<void> _openToolDialog({_DevToolItem? existing}) async {
    ...
    final licCtrl =
        SpellCheckTextEditingController(text: existing?.license ?? '');
    final usersCtrl =
        SpellCheckTextEditingController(text: existing?.assignedUsers ?? '');
    final statusCtrl =
        SpellCheckTextEditingController(text: existing?.status ?? 'Active');
    final expiryCtrl =
    ...
    await showDialog<void>(
      ...
      builder: (ctx) {
        ...
        final dialogWidth = (screen.width * 0.78).clamp(0.0, 900.0);
        final dialogMaxHeight = screen.height * 0.92;
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _dialogField('Category', catCtrl,
                              hint: 'IDE / VCS / Cloud / Design'),
                          const SizedBox(height: 16),
                          _dialogField('License', licCtrl,
                              hint: 'Enterprise / Premium / Team'),
                          const SizedBox(height: 16),
                          _dialogField('Assigned Users', usersCtrl),
                          const SizedBox(height: 16),
                          _dialogField('Status', statusCtrl,
                              hint: 'Active / Expired / Pending'),
                          const SizedBox(height: 16),
  ...
  Widget _buildQualityGatesRegister() {
    return _PanelShell(
      ...
      subtitle:
          'Track quality gates, criteria, methodology alignment, and approver accountability for all quality checkpoints.',
      trailing: OutlinedButton.icon(
        ...
        children: [
          ...
          if (_qualityItems.isEmpty)
            const Padding(
                ...
                child: Center(
                    child: Text(
                        'No quality gates registered. Click Add to create one.',
                        style: TextStyle(color: Color(0xFF6B7280)))))
          else
            ...List.generate(_qualityItems.length,
                (i) => _buildQualityRow(_qualityItems[i], i)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
              flex: 4,
              child: Text(item.criteria,
                  style:
                      const TextStyle(fontSize: 12, color: Color(0xFF475569)),
                  overflow: TextOverflow.ellipsis)),
          SizedBox(
              width: 120,
              child: Text(item.methodology,
                  style:
                      const TextStyle(fontSize: 12, color: Color(0xFF475569)))),
            ...
            child: Row(
              children: [
                IconButton(
                    icon: const Icon(Icons.edit_outlined,
                        size: 16, color: Color(0xFF64748B)),
                    onPressed: () => _openQualityDialog(existing: item),
                    padding: EdgeInsets.zero,
                    constraints:
                        const BoxConstraints(minWidth: 28, minHeight: 28)),
                IconButton(
                    icon: const Icon(Icons.delete_outline,
                        size: 16, color: Color(0xFFEF4444)),
                    onPressed: () =>
                        _confirmDelete('quality gate', item.id, () {
                          setState(() => _qualityItems
  ...
  Future<void> _openQualityDialog({_QualityGateItem? existing}) async {
    ...
    final critCtrl =
        SpellCheckTextEditingController(text: existing?.criteria ?? '');
    var selectedMethodology =
        _methodologyOptions.contains(existing?.methodology)
    ...
    if (existing?.targetDate != null && existing!.targetDate.isNotEmpty) {
      selectedTargetDate =
          DateFormat('MMM dd, yyyy').tryParse(existing.targetDate) ??
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              ...
              _dialogField('Criteria', critCtrl,
                  hint: 'e.g. All acceptance tests pass'),
              const SizedBox(height: 4),
            ...
            ElevatedButton(
                onPressed: () {
                  ...
                  setState(() {
                    if (isEdit) {
                      final idx =
                          _qualityItems.indexWhere((e) => e.id == item.id);
  ...
  Widget _buildSecurityBaselineRegister() {
    return _PanelShell(
      ...
      subtitle:
          'Track security controls, framework compliance, evidence, and review dates for all security baseline items.',
      trailing: OutlinedButton.icon(
        ...
        children: [
          ...
          if (_securityItems.isEmpty)
            const Padding(
                ...
                child: Center(
                    child: Text(
                        'No security controls registered. Click Add to create one.',
                        style: TextStyle(color: Color(0xFF6B7280)))))
          else
            ...List.generate(_securityItems.length,
                (i) => _buildSecurityRow(_securityItems[i], i)),
        ],
      ),
    );
  }
            ...
            child: Row(
              children: [
                IconButton(
                    icon: const Icon(Icons.edit_outlined,
                        size: 16, color: Color(0xFF64748B)),
                    onPressed: () => _openSecurityDialog(existing: item),
                    padding: EdgeInsets.zero,
                    constraints:
                        const BoxConstraints(minWidth: 28, minHeight: 28)),
                IconButton(
                    icon: const Icon(Icons.delete_outline,
                        size: 16, color: Color(0xFFEF4444)),
                    onPressed: () =>
                        _confirmDelete('security control', item.id, () {
                          setState(() => _securityItems
                              .removeWhere((e) => e.id == item.id));
                          _scheduleSave();
                        }),
                    padding: EdgeInsets.zero,
  ...
  Future<void> _openSecurityDialog({_SecurityBaselineItem? existing}) async {
    final isEdit = existing != null;
    final ctrlCtrl =
        SpellCheckTextEditingController(text: existing?.control ?? '');
    final frameFieldCtrl =
        SpellCheckTextEditingController(text: existing?.framework ?? '');
    final statusCtrl = SpellCheckTextEditingController(
        text: existing?.status ?? 'Not Started');
    final evidCtrl =
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          _dialogField('Framework', frameFieldCtrl,
              hint: 'NIST 800-53 / OWASP / ISO 27001'),
          _dialogField('Status', statusCtrl,
              hint: 'In Progress / Not Started / Planned'),
          ...
          ElevatedButton(
              onPressed: () {
                ...
                setState(() {
                  if (isEdit) {
                    final idx =
                        _securityItems.indexWhere((e) => e.id == item.id);
  ...
  Widget _buildApprovalGatesPanel() {
    return _PanelShell(
      ...
      subtitle:
          'Development readiness approval gates aligned with PMI PMBOK phase gates and organizational authority matrices. Each gate must be cleared before development can commence.',
      trailing: OutlinedButton.icon(
        ...
        children: _approvalGates.isEmpty
            ? [
                const Padding(
                    ...
                    child: Text('No approval gates configured.',
                        style: TextStyle(color: Color(0xFF6B7280))))
              ]
            : _approvalGates
                .map((gate) => _buildApprovalGateCard(gate))
                .toList(),
      ),
    );
  }
          ...
          Row(
            children: [
              ...
              IconButton(
                  icon: const Icon(Icons.edit_outlined,
                      size: 16, color: Color(0xFF64748B)),
                  onPressed: () => _openApprovalDialog(existing: gate),
                  padding: EdgeInsets.zero,
                  constraints:
              ...
              IconButton(
                  icon: const Icon(Icons.delete_outline,
                      size: 16, color: Color(0xFFEF4444)),
                  onPressed: () => _confirmDelete('approval gate', gate.id, () {
                        setState(() =>
                            _approvalGates.removeWhere((e) => e.id == gate.id));
                        _scheduleSave();
                      }),
                  padding: EdgeInsets.zero,
                  constraints:
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          _dialogField('Status', statusCtrl,
              hint: 'Not Started / In Progress / Approved'),
          ...
          ElevatedButton(
              onPressed: () {
                ...
                setState(() {
                  if (isEdit) {
                    final idx =
                        _approvalGates.indexWhere((e) => e.id == item.id);
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_development_set_up_notes'] ??
                'No data recorded.'),
      ],
    );
  }
