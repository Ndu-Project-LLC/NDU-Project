...
class _DocumentReviewMatrixScreenState
    extends State<DocumentReviewMatrixScreen> {
  ...
  Widget _buildTableHeader() {
    return Container(
      ...
      child: const Row(
        children: [
          SizedBox(
              width: 50, child: Center(child: Text('#', style: _headerStyle))),
          Expanded(
              flex: 2,
              child: Center(child: Text('DOCUMENT', style: _headerStyle))),
  ...
  Widget _buildContent(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _buildInfoRow(
            'Due Date',
            document.reviewDueDate != null
                ? document.reviewDueDate!.toLocal().toString().split(' ')[0]
                : 'Not set'),
        _buildInfoRow(
            'Primary Reviewer', document.primaryReviewerName ?? 'Not assigned'),
        _buildInfoRow(
            'Final Approver', document.finalApproverName ?? 'Not assigned'),
        if (document.reviewComments != null &&
            document.reviewComments!.isNotEmpty) ...[
          const SizedBox(height: 16),
...
class _AssignReviewerDialog extends StatefulWidget {
  final DocumentReviewItem document;
  final Function(String reviewerId, String reviewerName, ReviewerRole role)
      onAssign;
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: _selectedUserId.isEmpty
              ...
              : () {
                  widget.onAssign(
                      _selectedUserId, _selectedUserName, _selectedRole);
                },
