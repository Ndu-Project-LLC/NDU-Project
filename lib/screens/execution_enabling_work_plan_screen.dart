...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_enabling_work_plan_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionEnablingWorkPlanScreen extends StatefulWidget {
  ...
  static void open(BuildContext context) {
    context.push('/execution-enabling-work-plan');
  }

  @override
  State<ExecutionEnablingWorkPlanScreen> createState() =>
      _ExecutionEnablingWorkPlanScreenState();
}

class _ExecutionEnablingWorkPlanScreenState
    extends State<ExecutionEnablingWorkPlanScreen> {
  @override
  void initState() {
    super.initState();
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_enabling_work_plan'),
        ...
        Align(
          ...
          child: Row(
            ...
            children: [
              CsvTableImportButton(
                ...
                columns: [
                  CsvColumnSpec(
                      ...
                      required: true,
                      sampleValue: 'Site Access'),
                  CsvColumnSpec(
                      key: 'description',
                      label: 'Description',
                      required: true,
                      sampleValue: 'Road construction to site'),
                  CsvColumnSpec(
                      key: 'duration',
                      label: 'Duration',
                      required: true,
                  ...
                  CsvColumnSpec(
                      ...
                      label: 'Comments',
                      required: true,
                      sampleValue: 'Must be completed first'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _EnablingWorksPlanTable._getProjectIdStatic(context);
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported enabling work(s) successfully')),
                    );
                  }
                },
              ),
              ...
              AddRowButton(
                  onPressed: () =>
                      _EnablingWorksPlanTable.showAddDialog(context)),
            ],
          ),
        ),
...
class _EnablingWorksPlanTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<ExecutionEnablingWorkModel>>(
