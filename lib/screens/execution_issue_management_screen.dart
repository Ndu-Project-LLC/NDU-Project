...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_issue_management_screen'] ??
              'No data recorded.'),
    ],
  );
}
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_issue_management'),
...
class _IssuesManagementSection extends StatefulWidget {
  ...
  @override
  State<_IssuesManagementSection> createState() =>
      _IssuesManagementSectionState();
}
        ...
        Align(
          ...
          child: Row(
            ...
            children: [
              CsvTableImportButton(
                ...
                columns: const [
                  CsvColumnSpec(
                      ...
                      label: 'Issue Topic',
                      required: true,
                      sampleValue: 'Delay in permits'),
                  CsvColumnSpec(
                      key: 'description',
                      label: 'Description',
                      required: true,
                      sampleValue: 'Permit approval pending'),
                  CsvColumnSpec(
                      key: 'discipline',
                      label: 'Discipline',
                      required: true,
                  ...
                  CsvColumnSpec(
                      ...
                      label: 'Comments',
                      required: true,
                      sampleValue: 'Follow up with authorities'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _IssuesManagementTable._getProjectIdStatic(context);
                  if (projectId == null) return;
                  var imported = 0;
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content:
                              Text('Imported $imported issue(s) successfully')),
                    );
                  }
                },
              ),
              ...
              AddRowButton(
                  onPressed: () =>
                      _IssuesManagementTable.showAddDialog(context)),
            ],
          ),
        ),
