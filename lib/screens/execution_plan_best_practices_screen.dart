...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_plan_best_practices_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanBestPracticesScreen extends StatelessWidget {
  ...
  static void open(BuildContext context) {
    context.push('/execution-plan-best-practices');
  }
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_best_practices'),
                onNext: () => PlanningPhaseNavigation.goToNext(
                    context, 'execution_plan_best_practices'),
                onExportPdf: () => _exportPdf(context)),
            const SizedBox(height: 32),
            const SectionIntro(title: 'Execution Plan - Best Practices'),
              ...
              CsvTableImportButton(
                ...
                columns: [
                  CsvColumnSpec(
                      ...
                      label: 'Topic',
                      required: true,
                      sampleValue: 'Daily standup meetings'),
                  CsvColumnSpec(
                      key: 'description',
                      label: 'Description',
                      required: true,
                      sampleValue: 'Hold 15-min daily sync meetings'),
                  CsvColumnSpec(
                      key: 'discipline',
                      label: 'Discipline',
                      required: true,
                  ...
                  CsvColumnSpec(
                      key: 'impacted',
                      label: 'Impacted',
                      sampleValue: 'Team communication'),
                  CsvColumnSpec(
                      key: 'raisedBy',
                      label: 'Raised By',
                      required: true,
                      sampleValue: 'Scrum Master'),
                  CsvColumnSpec(
                      key: 'scheduleImpact',
                      label: 'Schedule Impact',
                      required: true,
                      sampleValue: 'Improved'),
                  CsvColumnSpec(
                      key: 'costImpact',
                      label: 'Cost Impact',
                      required: true,
                  ...
                  CsvColumnSpec(
                      ...
                      label: 'Comments',
                      required: true,
                      sampleValue: 'Proven effective in Phase 1'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _BestPracticesTable._getProjectIdStatic(context);
                  ...
                  for (final row in rows) {
                    try {
                      await ExecutionService.createChangeRequest(
                        ...
                        approved:
                            (row['approved'] ?? '').toLowerCase() == 'yes',
                        comments: row['comments'] ?? '',
                        llOrBp: 'BP',
                        impacted: (row['impacted'] ?? '').isEmpty
                            ? null
                            : row['impacted'],
                      );
                      imported++;
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported best practice(s) successfully')),
                    );
                  }
                },
              ),
...
class _BestPracticesTable extends StatelessWidget {
  ...
  static void showAddDialog(BuildContext context) {
    ...
    LessonsLearnedTable.showChangeRequestDialog(context, null, projectId, 'BP');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<ExecutionIssueModel>>(
