  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData
                  .planningNotes['execution_plan_communication_plan_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanCommunicationPlanScreen extends StatelessWidget {
  ...
  static void open(BuildContext context) {
    context.push('/execution-plan-communication-plan');
  }
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_communication_plan'),
                onNext: () => PlanningPhaseNavigation.goToNext(
                    context, 'execution_plan_communication_plan'),
                onExportPdf: () => _exportPdf(context)),
            const SizedBox(height: 32),
            const SectionIntro(title: 'Execution Plan - Communication Plan'),
        ...
        Align(
          ...
          child: Row(
            ...
            children: [
              CsvTableImportButton(
                ...
                columns: const [
                  CsvColumnSpec(
                      ...
                      label: 'Stakeholder',
                      required: true,
                      sampleValue: 'Project Sponsor'),
                  CsvColumnSpec(
                      key: 'infoType',
                      label: 'Info Type',
                      required: true,
                      sampleValue: 'Status Update'),
                  CsvColumnSpec(
                      key: 'frequency',
                      label: 'Frequency',
                      allowedValues: [
                  ...
                  CsvColumnSpec(
                      key: 'comments',
                      label: 'Comments',
                      sampleValue: 'Weekly progress report'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _CommunicationPlanTable._getProjectIdStatic(context);
                    ...
                    try {
                      await ExecutionService.createCommunicationPlan(
                        ...
                        frequency: row['frequency']?.isNotEmpty == true
                            ? row['frequency']!
                            : 'Weekly',
                        channel: row['channel']?.isNotEmpty == true
                            ? row['channel']!
                            : 'Email',
                        owner: row['owner'] ?? '',
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported communication entr(ies) successfully')),
                    );
                  }
                },
              ),
              ...
              AddRowButton(
                  onPressed: () =>
                      _CommunicationPlanTable.showAddDialog(context)),
            ],
          ),
        ),
...
class _CommunicationPlanTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<CommunicationPlanModel>>(
