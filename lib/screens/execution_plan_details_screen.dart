...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_plan_details_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanDetailsScreen extends StatefulWidget {
  ...
  @override
  State<ExecutionPlanDetailsScreen> createState() =>
      _ExecutionPlanDetailsScreenState();
}

class _ExecutionPlanDetailsScreenState
    extends State<ExecutionPlanDetailsScreen> {
  @override
  void initState() {
    super.initState();
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    YellowActionButton(
                      label: 'Next',
                      onPressed: () =>
                          ExecutionPlanDetailsScreen.openEarlyWorks(context),
                    ),
                  ],
                ),
              ),
              ...
              CsvTableImportButton(
                ...
                columns: const [
                  CsvColumnSpec(
                      ...
                      required: true,
                      sampleValue: 'Excavator'),
                  CsvColumnSpec(
                      key: 'description',
                      label: 'Description',
                      required: true,
                      sampleValue: 'Site preparation work'),
                  CsvColumnSpec(
                      key: 'cost', label: 'Cost', sampleValue: '5000'),
                  CsvColumnSpec(
                      key: 'comments',
                      label: 'Comments',
                      required: true,
                      sampleValue: 'Must complete before Phase 2'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _EarlyWorksTable._getProjectIdStatic(context);
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported early work(s) successfully')),
                    );
                  }
                },
              ),
