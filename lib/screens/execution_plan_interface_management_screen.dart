    ...
    sections: [
      ...
      PdfSection.text(
          ...
          projectData.planningNotes[
                  'execution_plan_interface_management_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanInterfaceManagementScreen extends StatelessWidget {
  ...
  static void open(BuildContext context) {
    context.push('/execution-interface-management');
  }
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_interface_management'),
                onNext: () => PlanningPhaseNavigation.goToNext(
                    context, 'execution_plan_interface_management'),
                onExportPdf: () => _exportPdf(context)),
            const SizedBox(height: 32),
            const SectionIntro(title: 'Execution Interface Management'),
        ...
        Align(
          ...
          child: Row(
            ...
            children: [
              CsvTableImportButton(
                ...
                columns: const [
                  ...
                  CsvColumnSpec(
                      ...
                      sampleValue: 'Physical'),
                  CsvColumnSpec(
                      key: 'partyA',
                      label: 'Party A',
                      required: true,
                      sampleValue: 'Contractor A'),
                  CsvColumnSpec(
                      key: 'partyB',
                      label: 'Party B',
                      required: true,
                      sampleValue: 'Contractor B'),
                  CsvColumnSpec(
                      key: 'status',
                      label: 'Status',
                      allowedValues: [
                  ...
                  CsvColumnSpec(
                      key: 'comments',
                      label: 'Comments',
                      sampleValue: 'Coordinate schedule'),
                ],
                onImport: (rows) async {
                  final projectId =
                      _InterfaceRegisterTable._getProjectIdStatic(context);
                  ...
                  for (final row in rows) {
                    try {
                      await ExecutionService.createInterfaceRegister(
                        ...
                        interfaceType: row['interfaceType']?.isNotEmpty == true
                            ? row['interfaceType']!
                            : 'Physical',
                        partyA: row['partyA'] ?? '',
                        partyB: row['partyB'] ?? '',
                        status: row['status']?.isNotEmpty == true
                            ? row['status']!
                            : 'Active',
                        frequency: row['frequency']?.isNotEmpty == true
                            ? row['frequency']!
                            : 'Daily',
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported interface entr(ies) successfully')),
                    );
                  }
                },
              ),
              ...
              AddRowButton(
                  onPressed: () =>
                      _InterfaceRegisterTable.showAddDialog(context)),
            ],
          ),
        ),
...
class _InterfaceRegisterTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<InterfaceRegisterModel>>(
