...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_plan_lessons_learned_screen'] ??
              'No data recorded.'),
    ],
  );
}
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_lessons_learned'),
                onNext: () => PlanningPhaseNavigation.goToNext(
                    context, 'execution_plan_lessons_learned'),
                onExportPdf: () => _exportPdf(context)),
            const SizedBox(height: 32),
            const SectionIntro(title: 'Execution Lessons Learned'),
              ...
              CsvTableImportButton(
                ...
                columns: const [
                  CsvColumnSpec(
                      ...
                      label: 'Topic',
                      required: true,
                      sampleValue: 'Late vendor delivery'),
                  CsvColumnSpec(
                      key: 'description',
                      label: 'Description',
                      required: true,
                      sampleValue: 'Vendor delivered 2 weeks late'),
                  CsvColumnSpec(
                      key: 'discipline',
                      label: 'Discipline',
                      required: true,
                      sampleValue: 'Procurement'),
                  CsvColumnSpec(
                      key: 'impacted',
                      label: 'Impacted',
                  ...
                  CsvColumnSpec(
                      ...
                      label: 'Comments',
                      required: true,
                      sampleValue: 'Add buffer for future orders'),
                ],
                onImport: (rows) async {
                  final projectId =
                      LessonsLearnedTable._getProjectIdStatic(context);
                  ...
                  if (context.mounted && imported > 0) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content: Text(
                              'Imported $imported lesson(s) learned successfully')),
                    );
                  }
                },
              ),
