...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_plan_solutions_screen'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanSolutionsScreen extends StatefulWidget {
  ...
  @override
  State<ExecutionPlanSolutionsScreen> createState() =>
      _ExecutionPlanSolutionsScreenState();
}

class _ExecutionPlanSolutionsScreenState
    extends State<ExecutionPlanSolutionsScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _autoGenerateNotesIfNeeded());
  }
  ...
  Future<void> _autoGenerateNotesIfNeeded() async {
    ...
    final ctx = ProjectDataHelper.buildExecutivePlanContext(data,
    ...
    try {
      ...
      final result = await ai.generateExecutionPlanSectionFields(
        ...
        fields: {
          'notes':
              'Execution strategy notes capturing approach, sequence of activities, and key execution decisions'
        },
      );
      if (!mounted) return;
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_strategy'),
                  ...
                  CsvTableImportButton(
                    ...
                    columns: [
                      CsvColumnSpec(
                          ...
                          label: 'Tool',
                          required: true,
                          sampleValue: 'Hammer Drill'),
                      CsvColumnSpec(
                          key: 'description',
                          label: 'Description',
                          required: true,
                          sampleValue: 'Heavy-duty drilling tool'),
                      CsvColumnSpec(
                          key: 'source',
                          label: 'Source',
                          required: true,
                          sampleValue: 'Vendor A'),
                      CsvColumnSpec(
                          key: 'comments',
                          label: 'Comments',
                          required: true,
                          sampleValue: 'Required for Phase 1'),
                    ],
                    onImport: (rows) async {
                      final projectId =
                          _ExecutionPlanTable._getProjectIdStatic(context);
                        ...
                        try {
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                  content: Text('Error importing row: $e')),
                            );
                          }
                        }
                      }
                      if (context.mounted && imported > 0) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text(
                                  'Imported $imported tool(s) successfully')),
                        );
                      }
                    },
                  ),
...
class _ExecutionPlanTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<ExecutionToolModel>>(
