...
Future<void> _exportPdf(BuildContext context) async {
  ...
  await PdfExportHelper.exportScreenPdf(
    ...
    sections: [
      ...
      PdfSection.text(
          'Notes',
          projectData.planningNotes['execution_stakeholder_identification'] ??
              'No data recorded.'),
    ],
  );
}
...
class ExecutionPlanStakeholderIdentificationScreen extends StatelessWidget {
  ...
  static void open(BuildContext context) {
    context.push('/execution-plan-stakeholder-identification');
  }
          ...
          children: [
            ExecutionPlanHeader(
                onBack: () => PlanningPhaseNavigation.goToPrevious(
                    context, 'execution_plan_stakeholder_identification'),
                onNext: () => PlanningPhaseNavigation.goToNext(
                    context, 'execution_plan_stakeholder_identification'),
                onExportPdf: () => _exportPdf(context)),
            const SizedBox(height: 32),
            const SectionIntro(title: 'Execution Stakeholder Identification'),
              ...
              CsvTableImportButton(
                ...
                columns: const [
                  ...
                  CsvColumnSpec(
                      key: 'keyConcerns',
                      label: 'Key Concerns',
                      sampleValue: 'Environmental impact'),
                  CsvColumnSpec(
                      key: 'engagementStrategy',
                      label: 'Engagement Strategy',
                      sampleValue: 'Regular meetings'),
                  CsvColumnSpec(
                      key: 'comments',
                      label: 'Comments',
                      sampleValue: 'Monthly updates required'),
                ],
                onImport: (rows) async {
                  var imported = 0;
                  ...
                  if (imported > 0) {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(
                                'Imported $imported stakeholder(s) successfully')),
                      );
                    }
                  }
                },
