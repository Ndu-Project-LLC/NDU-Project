...
class _InitiationPhaseScreenState extends State<InitiationPhaseScreen> {
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text('Business Case',
            businessCase.isEmpty ? 'No data recorded.' : businessCase),
      ],
    );
  }
  ...
  String _formatSuggestionError(Object error) {
    ...
    if (lower.contains('rate limit')) {
      return 'AI suggestions are temporarily unavailable. Your content is safe—please try again later.';
    }
  ...
  Widget _buildSuggestionPanel({
    ...
    final isRateLimited = (error ?? '').toLowerCase().contains('rate limit');
    final borderColor = hasError && !isRateLimited
        ? Colors.red.withValues(alpha: 0.2)
        : Colors.grey.withValues(alpha: 0.2);
    final canRefresh = !loading && OpenAiConfig.isConfigured && !isRateLimited;
        ...
        children: [
          ...
          if (!loading && hasError)
            Text(
              ...
              style: TextStyle(
                fontSize: 13,
                color: isRateLimited ? Colors.black54 : Colors.red[600],
              ),
            ),
