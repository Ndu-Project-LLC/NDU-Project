...
class _LandingPageScreenState extends State<LandingPageScreen> {
  ...
  Widget _navLink(String label, VoidCallback onTap) {
    return Padding(
      ...
      child: TextButton(
        onPressed: onTap,
        child: Text(label, style: const TextStyle(color: _textSecondary, fontSize: 13, fontWeight: FontWeight.w500, fontFamily: appFontFamily)),
      ),
    );
  }
...
class _HeroSection extends StatelessWidget {
        ...
        child: ConstrainedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                // Badge
                Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6), decoration: BoxDecoration(color: _blue.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20), border: Border.all(color: _blue.withValues(alpha: 0.3))), child: const Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                // Headline
                const Text('42% of Projects Fail to meet original scope.\nFix Project Failure Before It Starts',
                ...
                // Subheadline
                const Text('Ndu Project is a Project Delivery Operating System — a SaaS platform that integrates AI, analytics, and human decision making to deliver projects from initiation through completion.',
                  textAlign: TextAlign.center,
  ...
  Widget _phaseNode(String label, Color color, [IconData? icon]) {
    return Column(children: [
      ...
      Text(label, style: const TextStyle(color: _textPrimary, fontSize: 13, fontWeight: FontWeight.w700, fontFamily: appFontFamily)),
    ]);
  }

  Widget _arrow() => const Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Icon(Icons.arrow_forward, color: _textMuted, size: 20));
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        ...
        child: Column(children: [
          const Text('Built from real-world delivery experience across global enterprises and high-growth organizations',
            textAlign: TextAlign.center,
          ...
          const SizedBox(height: 32),
          // Trusted by
          const Text('TRUSTED BY', style: TextStyle(color: _textMuted, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 2, fontFamily: appFontFamily)),
          const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('Projects Don\'t Fail in Execution.\nThey Fail Before Execution Begins',
            textAlign: TextAlign.center,
          ...
          const SizedBox(height: 16),
          const Text('Most project tools focus on tracking work after it starts. But by then, the most critical decisions have already been made… and often made poorly.',
            textAlign: TextAlign.center,
          ...
          const SizedBox(height: 40),
          Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: _red.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(12), border: Border.all(color: _red.withValues(alpha: 0.2))), child: const Text('The issue isn\'t execution. It\'s the lack of a system governing the full lifecycle.', textAlign: TextAlign.center, style: TextStyle(color: _red, fontSize: 16, fontWeight: FontWeight.w600, fontFamily: appFontFamily))),
        ]),
      )),
    );
  }
...
class _SolutionSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('A New Category: Project Delivery Operating System',
            textAlign: TextAlign.center,
            style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 16),
          const Text('Ndu Project replaces disconnected tools with a unified system that governs how projects are defined, planned, and delivered.',
            textAlign: TextAlign.center,
            style: TextStyle(color: _textSecondary, fontSize: 16, height: 1.6, fontFamily: appFontFamily)),
          const SizedBox(height: 40),
          // Capability grid
  ...
  Widget _compareRow(String trad, String ndu) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: const BoxDecoration(border: Border(top: BorderSide(color: _border))), child: Row(children: [Expanded(child: Row(children: [const Icon(Icons.close, color: _red, size: 14), const SizedBox(width: 6), Text(trad, style: const TextStyle(color: _textSecondary, fontSize: 13, fontFamily: appFontFamily))])), Expanded(child: Row(children: [const Icon(Icons.check, color: _green, size: 14), const SizedBox(width: 6), Text(ndu, style: const TextStyle(color: _textPrimary, fontSize: 13, fontWeight: FontWeight.w500, fontFamily: appFontFamily))]))]));
  }
}
...
class _HowItWorksSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('How Ndu Project Delivers Results', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 8),
          const Text('For projects, programs, and portfolios — Agile, Waterfall, and Hybrid', textAlign: TextAlign.center, style: TextStyle(color: _textSecondary, fontSize: 15, fontFamily: appFontFamily)),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.center, children: steps.map((s) => Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            ...
            if (s.$1 != '03') const Padding(padding: EdgeInsets.symmetric(horizontal: 24, vertical: 40), child: Icon(Icons.arrow_forward, color: _textMuted, size: 28)),
          ])).toList()),
        ]),
      )),
    );
  ...
  Widget _stepCard(String num, String title, String desc, IconData icon, Color color) {
    return Container(width: 240, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: _surfaceCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _border)), child: Column(children: [
      ...
      const SizedBox(height: 16),
      Text(title, style: const TextStyle(color: _textPrimary, fontSize: 18, fontWeight: FontWeight.w700, fontFamily: appFontFamily)),
      const SizedBox(height: 8),
      Text(desc, textAlign: TextAlign.center, style: const TextStyle(color: _textSecondary, fontSize: 13, height: 1.5, fontFamily: appFontFamily)),
    ]));
  }
}
...
class _DifferentiatorsSection extends StatelessWidget {
  ...
  Widget _stat(String value, String label, Color color) {
    return Container(margin: const EdgeInsets.symmetric(horizontal: 24), child: Column(children: [Text(value, style: TextStyle(color: color, fontSize: 40, fontWeight: FontWeight.w900, fontFamily: appFontFamily)), const SizedBox(height: 4), Text(label, textAlign: TextAlign.center, style: const TextStyle(color: _textSecondary, fontSize: 12, fontFamily: appFontFamily))]));
  }

  Widget _expBadge(String label, IconData icon) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: _surfaceCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: _border)), child: Row(children: [Icon(icon, color: _gold, size: 14), const SizedBox(width: 6), Text(label, style: const TextStyle(color: _textSecondary, fontSize: 12, fontFamily: appFontFamily))]));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('Projects We Help You Deliver', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 8),
          const Text('Industry-agnostic — project delivery is the heart of every business', textAlign: TextAlign.center, style: TextStyle(color: _textSecondary, fontSize: 15, fontFamily: appFontFamily)),
          const SizedBox(height: 40),
...
class _KazAISection extends StatelessWidget {
      ...
      child: Center(child: ConstrainedBox(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: _purple.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)), child: const Text('KAZ AI', style: TextStyle(color: _purpleLight, fontSize: 12, fontWeight: FontWeight.w700, fontFamily: appFontFamily))),
            const SizedBox(height: 16),
            const Text('Your knowledgeable project delivery sidekick', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
            const SizedBox(height: 16),
            const Text('KAZ AI provides intelligent suggestions, continuity across the project lifecycle, and core AI capabilities within the platform.', textAlign: TextAlign.center, style: TextStyle(color: _textSecondary, fontSize: 15, height: 1.6, fontFamily: appFontFamily)),
            const SizedBox(height: 24),
          ...
          Expanded(child: Container(padding: const EdgeInsets.all(32), decoration: BoxDecoration(color: _surfaceCard, borderRadius: BorderRadius.circular(20), border: Border.all(color: _purple.withValues(alpha: 0.2))), child: Column(children: [
            Container(width: 64, height: 64, decoration: BoxDecoration(gradient: const LinearGradient(colors: [_purple, _blue]), borderRadius: BorderRadius.circular(16)), child: const Icon(Icons.psychology, color: Colors.white, size: 32)),
            const SizedBox(height: 16),
            const Text('KAZ AI', style: TextStyle(color: _textPrimary, fontSize: 24, fontWeight: FontWeight.w800, fontFamily: appFontFamily)),
            const SizedBox(height: 4),
            const Text('Project Delivery Intelligence', style: TextStyle(color: _purpleLight, fontSize: 13, fontFamily: appFontFamily)),
            const SizedBox(height: 24),
  ...
  Widget _aiFeature(String title, String desc) {
    return Padding(padding: const EdgeInsets.only(bottom: 16), child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [const Icon(Icons.check_circle, color: _purple, size: 18), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [Text(title, textAlign: TextAlign.center, style: const TextStyle(color: _textPrimary, fontSize: 14, fontWeight: FontWeight.w600, fontFamily: appFontFamily)), Text(desc, textAlign: TextAlign.center, style: const TextStyle(color: _textSecondary, fontSize: 12, fontFamily: appFontFamily))]))]));
  }

  Widget _chatBubble(String text) {
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), decoration: BoxDecoration(color: _purple.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(12)), child: Row(children: [const Icon(Icons.auto_awesome, color: _purple, size: 14), const SizedBox(width: 8), Expanded(child: Text(text, style: const TextStyle(color: _textPrimary, fontSize: 12, fontFamily: appFontFamily)))]));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('What You Achieve with PDOS', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 40),
          Wrap(spacing: 20, runSpacing: 20, alignment: WrapAlignment.center, children: benefits.map((b) => _benefitCard(b.$1, b.$2, b.$3)).toList()),
          const SizedBox(height: 32),
          Container(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12), decoration: BoxDecoration(color: _green.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: _green.withValues(alpha: 0.2))), child: const Text('Up to 30% reduction in rework', style: TextStyle(color: _green, fontSize: 18, fontWeight: FontWeight.w700, fontFamily: appFontFamily))),
        ]),
      )),
    );
  }
    ...
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        ...
        child: Column(children: [
          ...
          // Main Title
          const Text('Simple, Scalable Pricing for Every Level of Project Delivery',
            textAlign: TextAlign.center,
            style: TextStyle(
...
class _ServicesSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        constraints: const BoxConstraints(maxWidth: 1200),
        child: Column(children: [
          const Text('Services', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 32, fontWeight: FontWeight.w700, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 8),
  ...
  Widget _serviceCard(String title, String desc, IconData icon, Color color, String cta, VoidCallback onTap) {
    return Expanded(child: Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: _surfaceCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: _border)), child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
      ...
      const SizedBox(height: 16),
      Text(title, textAlign: TextAlign.center, style: const TextStyle(color: _textPrimary, fontSize: 18, fontWeight: FontWeight.w700, fontFamily: appFontFamily)),
      const SizedBox(height: 8),
      Text(desc, textAlign: TextAlign.center, style: const TextStyle(color: _textSecondary, fontSize: 13, height: 1.5, fontFamily: appFontFamily)),
      const SizedBox(height: 20),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: ConstrainedBox(
        ...
        child: Column(children: [
          const Text('Ready to Transform How You Deliver Projects?', textAlign: TextAlign.center, style: TextStyle(color: _textPrimary, fontSize: 36, fontWeight: FontWeight.w800, letterSpacing: -0.5, fontFamily: appFontFamily)),
          const SizedBox(height: 16),
...
class _FooterSection extends StatelessWidget {
      ...
      child: Center(child: ConstrainedBox(
        ...
        child: Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            // Contact
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
              const Text('Contact', style: TextStyle(color: _textPrimary, fontSize: 13, fontWeight: FontWeight.w700, fontFamily: appFontFamily)),
              const SizedBox(height: 12),
          ...
          const SizedBox(height: 20),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Text('© 2026 Ndu Project. All rights reserved.', style: TextStyle(color: _textMuted, fontSize: 11, fontFamily: appFontFamily)),
            Row(children: [TextButton(onPressed: () {}, child: const Text('Privacy', style: TextStyle(color: _textMuted, fontSize: 11, fontFamily: appFontFamily))), TextButton(onPressed: () {}, child: const Text('Terms', style: TextStyle(color: _textMuted, fontSize: 11, fontFamily: appFontFamily)))]),
          ]),
        ]),
      )),
    );
  ...
  Widget _footerCol(String title, List<String> items) {
    return Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
      Text(title, style: const TextStyle(color: _textPrimary, fontSize: 13, fontWeight: FontWeight.w700, fontFamily: appFontFamily)),
      const SizedBox(height: 12),
      ...items.map((item) => Padding(padding: const EdgeInsets.only(bottom: 8), child: TextButton(onPressed: () {}, style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: Size.zero), child: Text(item, style: const TextStyle(color: _textSecondary, fontSize: 12, fontFamily: appFontFamily))))),
    ]));
  }
  ...
  Widget _contactRow(IconData icon, String text, VoidCallback onTap) {
    return Padding(padding: const EdgeInsets.only(bottom: 8), child: InkWell(onTap: onTap, child: Row(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.center, children: [Icon(icon, color: _textMuted, size: 14), const SizedBox(width: 8), Flexible(child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: _textSecondary, fontSize: 11, height: 1.4, fontFamily: appFontFamily)))])));
  }
}
