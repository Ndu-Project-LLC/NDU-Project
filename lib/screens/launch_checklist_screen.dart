...
class _LaunchChecklistScreenState extends State<LaunchChecklistScreen> {
  ...
  Future<void> _autoPopulateFromPriorPhases() async {
    ...
    try {
      ...
      for (final s in cp.planningSprints) {
        final goal = s['goal']?.toString() ?? s['title']?.toString() ?? '';
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final filename =
          'launch_checklist_${projectName.replaceAll(' ', '_')}_$stamp.pdf';
              ...
              children: [
                ...
                ExecutionActionBar(
                  actions: [
                    ...
                    ExecutionActionItem(
                      label: _selectedView == 'full'
                          ? 'Summary View'
                          : 'Full View',
                      icon: _selectedView == 'full'
                          ? Icons.summarize_outlined
                          : Icons.list_alt,
                      tone: ExecutionActionTone.secondary,
                      onPressed: () => setState(() {
                        _selectedView =
                            _selectedView == 'full' ? 'summary' : 'full';
                      }),
                    ),
                    ExecutionActionItem(
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'task',
            label: 'Task',
            sampleValue: 'Cutover rehearsals signed off'),
        CsvColumnSpec(
            key: 'detail',
            label: 'Detail',
            sampleValue: 'Dry run #2 captured follow-up items'),
        CsvColumnSpec(
            key: 'owner', label: 'Owner', sampleValue: 'Operations lead'),
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Complete', 'In review', 'Pending'],
            hint: 'Status'),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'approval',
            label: 'Approval',
            sampleValue: 'Cutover sign-off'),
        CsvColumnSpec(
            key: 'detail',
            label: 'Detail',
            sampleValue: 'Delivery and ops leads approved'),
        CsvColumnSpec(
            key: 'approver', label: 'Approver', sampleValue: 'Ops Director'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
            allowedValues: ['Complete', 'In review', 'Pending']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
      ...
      cellBuilder: (context, i) {
        ...
        return LaunchDataRow(
          ...
          onDelete: () async {
            final confirmed =
                await launchConfirmDelete(context, itemName: 'approval');
            if (!confirmed || !mounted) return;
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Complete', 'Upcoming', 'In progress', 'Delayed'],
            hint: 'Status'),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'milestone',
            label: 'Milestone',
            sampleValue: 'Go / no-go rehearsal'),
        CsvColumnSpec(
            key: 'detail',
            label: 'Detail',
            sampleValue: 'Dry run with scenario walk-through'),
        CsvColumnSpec(key: 'due', label: 'Due', sampleValue: 'Aug 11'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Upcoming',
            allowedValues: ['Complete', 'Upcoming', 'In progress', 'Delayed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
      ...
      cellBuilder: (context, i) {
        ...
        return LaunchDataRow(
          ...
          onDelete: () async {
            final confirmed =
                await launchConfirmDelete(context, itemName: 'milestone');
            if (!confirmed || !mounted) return;
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Complete', 'In progress', 'Upcoming', 'Delayed'],
            hint: 'Status'),
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'stage',
            label: 'Stage',
            sampleValue: 'Final readiness review'),
        CsvColumnSpec(
            key: 'detail',
            label: 'Detail',
            sampleValue: 'All cutover artefacts verified'),
        CsvColumnSpec(key: 'date', label: 'Date', sampleValue: 'Aug 10'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Upcoming',
            allowedValues: ['Complete', 'In progress', 'Upcoming', 'Delayed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
      ...
      cellBuilder: (context, i) {
        ...
        return LaunchDataRow(
          ...
          onDelete: () async {
            final confirmed =
                await launchConfirmDelete(context, itemName: 'timeline stage');
            if (!confirmed || !mounted) return;
