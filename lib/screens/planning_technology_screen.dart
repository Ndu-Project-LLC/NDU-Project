...
class _PlanningTechnologyScreenState extends State<PlanningTechnologyScreen> {
  ...
  Future<void> _openIntegrationDialog(
    ...
    final name = SpellCheckTextEditingController(
        text: existing['name']?.toString() ?? '');
    final description = SpellCheckTextEditingController(
...
// skipped from the top of the page either.
  ...
  Widget _buildTabsBar() {
    ...
    final flowTabs = [
      for (final tab in _TechnologyTab.values)
        FlowTab(id: tab.name, label: tab.label),
    ];
              ...
              child: SectionProgressBar(
                ...
                onOpenTab: (id) => setState(() {
                  final tab = _TechnologyTab.values.firstWhere(
                      (t) => t.name == id,
                      orElse: () => _TechnologyTab.inventory);
                  _selectedTab = tab;
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_technology_notes'] ??
                'No data recorded.'),
      ],
    );
  }
