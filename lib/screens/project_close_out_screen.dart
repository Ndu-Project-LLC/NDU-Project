...
class _ProjectCloseOutScreenState extends State<ProjectCloseOutScreen> {
    ...
    return LaunchDataTable(
      ...
      columns: const [
        LaunchColumn(
            ...
            dropdownItems: LaunchCloseOutCheckItem.categories),
        LaunchColumn(
            label: 'Item',
            flexible: true,
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'item',
            label: 'Item',
            sampleValue: 'Verify final deliverables'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
  ...
  Widget _buildApprovalsPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'role', label: 'Role', sampleValue: 'Project Sponsor'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
        ...
        CsvColumnSpec(
            key: 'notes', label: 'Notes', sampleValue: 'Awaiting review'),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
  ...
  Widget _buildArchivePanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'repository', label: 'Repository', sampleValue: 'GitHub Repo'),
        CsvColumnSpec(
            key: 'documentType', label: 'Type', sampleValue: 'Source Code'),
        CsvColumnSpec(
            key: 'retentionPeriod', label: 'Retention', sampleValue: '5 years'),
        CsvColumnSpec(
            key: 'accessChange',
            label: 'Access Change',
            sampleValue: 'Read-only'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
  ...
  Future<void> _autoPopulateFromPriorPhases() async {
    ...
    try {
      ...
      if (cp.totalPlannedBudget > 0 &&
          !checklistExisting.contains('Budget reconciliation')) {
        newChecklistItems.add(LaunchCloseOutCheckItem(
          category: 'Finance',
          item: 'Budget reconciliation',
          status: cp.budgetVariance.abs() < cp.totalPlannedBudget * 0.05
              ? 'Complete'
              : 'Pending',
  ...
  Widget _buildLaunchInsights() {
    ...
    return LaunchInsightsHeader(
      ...
      sectionSubtitle:
          'Final documentation, archive, lessons learned, and stakeholder sign-off',
      ...
      kpiTiles: [
        ...
        LaunchKpiTile(
          ...
          delta:
              '${projectData.frontEndPlanning.riskRegisterItems.where((r) => r.status.toLowerCase() == 'closed').length} closed',
        ),
        LaunchKpiTile(
          label: 'Allowances',
