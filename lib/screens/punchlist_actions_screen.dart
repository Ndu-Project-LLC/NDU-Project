 ...
 */
  ...
  Future<void> _loadFromFirestore() async {
    ...
    try {
      ...
      final fieldItems =
          _PunchlistInsight.fromList(data['fieldExecutionItems']);
      ...
      if (distData != null && distData is List && distData.isNotEmpty) {
        _distributionRows = distData
            .map((e) => _DistributionRow.fromMap(e as Map<String, dynamic>))
            .toList();
      }
      if (velData != null && velData is List && velData.isNotEmpty) {
        _velocityRows = velData
            .map((e) => _ActionVelocityRow.fromMap(e as Map<String, dynamic>))
            .toList();
      }
      ...
      if (capData != null && capData is List && capData.isNotEmpty) {
        _capacityHealthRows = capData
            .map((e) => _CapacityHealthRow.fromMap(e as Map<String, dynamic>))
            .toList();
      }
      if (shiftData != null && shiftData is List && shiftData.isNotEmpty) {
        _shiftCoverageRows = shiftData
            .map((e) => _ShiftCoverageRow.fromMap(e as Map<String, dynamic>))
            .toList();
      }
      if (!hasContent) {
  ...
  Future<void> _saveToFirestore() async {
    ...
    try {
      await _docRef(projectId).set({
        ...
        'capacityHealthRows':
            _capacityHealthRows.map((e) => e.toMap()).toList(),
  ...
  Future<void> _autoPopulateFromAi() async {
    ...
    setState(() {
      _priorityItems = priority.isNotEmpty ? priority : _defaultPriorityItems();
      _technicalInsights =
          technical.isNotEmpty ? technical : _defaultTechnicalInsights();
      _remediationItems =
          remediation.isNotEmpty ? remediation : _defaultRemediationItems();
  ...
  List<_ShiftCoverageRow> _defaultShiftCoverageRows() => [
        ...
        const _ShiftCoverageRow(
            ...
            nextRotation: 'May 8',
            supervisor: 'Night Lead',
            riskFlag: 'High',
            status: 'Understaffed',
            lastUpdated: '30 min ago'),
        const _ShiftCoverageRow(
            shift: 'Weekend Coverage (Sat-Sun)',
            requiredHeadcount: 12,
            actualHeadcount: 10,
            coveragePercent: 83,
            gap: 2,
            shiftPattern: 'Alternate weekends',
            overtimeHrs: 8,
            contractorFill: 1,
            agencyStaff: 0,
            absenceCount: 2,
            complianceStatus: 'Compliant',
            nextRotation: 'May 11',
            supervisor: 'Weekend Lead',
            riskFlag: 'Medium',
            status: 'Covered',
            lastUpdated: '2 hrs ago'),
        const _ShiftCoverageRow(
            shift: 'Emergency Response On-Call',
            requiredHeadcount: 6,
            actualHeadcount: 6,
            coveragePercent: 100,
            gap: 0,
            shiftPattern: 'Rotating standby',
            overtimeHrs: 4,
            contractorFill: 0,
            agencyStaff: 0,
            absenceCount: 0,
            ...
            lastUpdated: '4 hrs ago'),
        const _ShiftCoverageRow(
            shift: 'QA Inspection Team',
            requiredHeadcount: 8,
            actualHeadcount: 6,
            coveragePercent: 75,
            gap: 2,
            shiftPattern: '5 on / 2 off',
            overtimeHrs: 10,
            contractorFill: 1,
            agencyStaff: 1,
            absenceCount: 3,
  ...
  Widget _buildDistributionCard() {
    final grandOpen =
        _distributionRows.fold<int>(0, (sum, r) => sum + r.openItems);
    final grandClosed =
        _distributionRows.fold<int>(0, (sum, r) => sum + r.closed);
    final grandTotal = grandOpen + grandClosed;
    final grandPct = grandTotal > 0 ? (grandClosed / grandTotal * 100) : 0.0;
    return _panel(
      title: 'Item distribution',
      subtitle:
          'Punchlist item severity breakdown by workstream category with ownership tracking and closure metrics.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Summary bar
          Container(
            ...
            child: Row(
              children: [
                ...
                _summaryMetric(
                    label: 'Completion',
                    value: '${grandPct.toStringAsFixed(1)}%',
                    ...
                    rows: _distributionRows.asMap().entries.map((entry) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(_numberCell(
                            '${row.openItems}', const Color(0xFFF59E0B))),
                        DataCell(_numberCell(
                            '${row.critical}',
                            row.critical > 0
                        ...
                        DataCell(_numberCell(
                            ...
                            row.high > 0
                                ...
                                : const Color(0xFF94A3B8))),
                        DataCell(_numberCell(
                            '${row.medium}', const Color(0xFFF59E0B))),
                        DataCell(
                            _numberCell('${row.low}', const Color(0xFF22C55E))),
                        DataCell(_numberCell(
                            '${row.closed}', const Color(0xFF22C55E))),
                        DataCell(Container(
                        ...
                        DataCell(Text(
                            row.lastUpdated.isNotEmpty ? row.lastUpdated : '-',
                            style: const TextStyle(
                        ...
                        DataCell(Row(
                          ...
                          children: [
                            ...
                            IconButton(
                                ...
                                onPressed: () => _deleteDistributionRow(idx),
                                splashRadius: 18,
                                tooltip: 'Delete'),
                            IconButton(
                              onPressed: () {
  ...
  Widget _buildActionVelocityCard() {
    ...
    final totalClosed =
        _velocityRows.fold<int>(0, (sum, r) => sum + r.closedThisSprint);
    final avgVelocity = _velocityRows.isNotEmpty
        ? _velocityRows.fold<int>(0, (sum, r) => sum + r.velocity) /
            _velocityRows.length
        : 0.0;
    final avgCycle = _velocityRows.isNotEmpty
        ? _velocityRows.fold<double>(0.0, (sum, r) => sum + r.avgCycleTime) /
            _velocityRows.length
        : 0.0;
    return _panel(
      title: 'Action velocity',
      subtitle:
          'Workstream throughput momentum measured across sprint boundaries with trend indicators and cycle time analysis.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Summary bar
          Container(
            ...
            child: Row(
              children: [
                ...
                _summaryMetric(
                    label: 'Avg Velocity',
                    value: '${avgVelocity.toStringAsFixed(0)}%',
                    color: const Color(0xFF2563EB)),
                const SizedBox(width: 20),
                _summaryMetric(
                    label: 'Avg Cycle Time',
                    value: '${avgCycle.toStringAsFixed(1)}d',
                  ...
                  child: DataTable(
                    ...
                    rows: _velocityRows.asMap().entries.map((entry) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(_numberCell(
                            '${row.openItems}', const Color(0xFFF59E0B))),
                        DataCell(_numberCell('${row.closedThisSprint}',
                            const Color(0xFF22C55E))),
                        DataCell(Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                        ...
                        DataCell(Text('${row.throughput.toStringAsFixed(1)}/sp',
                            style: const TextStyle(
                              fontWeight: FontWeight.w700,
                              color: Color(0xFF475569),
                        ...
                        DataCell(Container(
                          ...
                          decoration: BoxDecoration(
                            color: row.avgCycleTime <= 3.0
                                ? const Color(0xFFF0FDF4)
                                : row.avgCycleTime <= 5.0
                                    ? const Color(0xFFFFFBEB)
                                    : const Color(0xFFFEF2F2),
                            borderRadius: const BorderRadius.circular(8),
                          ),
                          child: Text('${row.avgCycleTime.toStringAsFixed(1)}d',
                              style: TextStyle(
                                fontWeight: FontWeight.w700,
                                fontFeatures: const [
                                  FontFeature.tabularFigures()
                        ...
                        DataCell(Row(
                          ...
                          children: [
                            ...
                            IconButton(
                                icon: const Icon(Icons.delete_outline,
                                    size: 18, color: Color(0xFFEF4444)),
                                onPressed: () => _deleteVelocityRow(idx),
                                splashRadius: 18,
                                tooltip: 'Delete'),
                            IconButton(
                              onPressed: () {
  ...
  Widget _buildCapacityHealthCard() {
    final totalPlanned =
        _capacityHealthRows.fold<double>(0.0, (sum, r) => sum + r.plannedFte);
    final totalAllocated =
        _capacityHealthRows.fold<double>(0.0, (sum, r) => sum + r.allocatedFte);
    final totalAvailable =
        _capacityHealthRows.fold<double>(0.0, (sum, r) => sum + r.availableFte);
    final avgUtilization = _capacityHealthRows.isNotEmpty
        ? _capacityHealthRows.fold<int>(0, (sum, r) => sum + r.utilization) /
            _capacityHealthRows.length
        : 0.0;
    final avgProductivity = _capacityHealthRows.isNotEmpty
        ? _capacityHealthRows.fold<int>(
                0, (sum, r) => sum + r.productivityIndex) /
            _capacityHealthRows.length
        : 0.0;
    final totalOvertime =
        _capacityHealthRows.fold<int>(0, (sum, r) => sum + r.overtimeHrs);
    final highRiskCount =
        _capacityHealthRows.where((r) => r.riskLevel == 'High').length;
    return _panel(
      title: 'Capacity Health',
      subtitle:
          'Workforce FTE planning vs allocation, utilization burn rate, productivity index, absenteeism, backlog capacity, and cost variance risk assessment across project teams.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Summary bar
          Container(
            ...
            child: Wrap(
              ...
              children: [
                _summaryMetric(
                    label: 'Planned FTE',
                    value: totalPlanned.toStringAsFixed(1),
                    color: const Color(0xFF7C3AED)),
                _summaryMetric(
                    label: 'Allocated FTE',
                    value: totalAllocated.toStringAsFixed(1),
                    color: const Color(0xFF1E293B)),
                _summaryMetric(
                    label: 'Available FTE',
                  ...
                  child: DataTable(
                    ...
                    rows: _capacityHealthRows.asMap().entries.map((entry) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(_numberCell(row.plannedFte.toStringAsFixed(1),
                            const Color(0xFF7C3AED))),
                        DataCell(_numberCell(
                            row.allocatedFte.toStringAsFixed(1),
                            const Color(0xFF1E293B))),
                        DataCell(_numberCell(
                            row.availableFte.toStringAsFixed(1),
                            const Color(0xFF22C55E))),
                        DataCell(Container(
                          padding: const EdgeInsets.symmetric(
                              horizontal: 6, vertical: 2),
                          decoration: BoxDecoration(
                            color: row.fteVariance < -1.0
                                ? const Color(0xFFFEF2F2)
                                : row.fteVariance < 0
                                    ? const Color(0xFFFFFBEB)
                                    : const Color(0xFFF0FDF4),
                          ...
                          child: Text(
                              '${row.fteVariance > 0 ? '+' : ''}${row.fteVariance.toStringAsFixed(1)}',
                              style: TextStyle(
                                fontWeight: FontWeight.w700,
                                fontFeatures: const [
                                  FontFeature.tabularFigures()
                        ...
                        DataCell(_numberCell(
                            row.overallocated.toStringAsFixed(1),
                            row.overallocated > 0
                                ? const Color(0xFFEF4444)
                                : const Color(0xFF94A3B8))),
                        DataCell(Container(
                        ...
                        DataCell(Container(
                          padding: const EdgeInsets.symmetric(
                              horizontal: 6, vertical: 2),
                          decoration: BoxDecoration(
                            color: row.productivityIndex >= 100
                                ? const Color(0xFFF0FDF4)
                                : row.productivityIndex >= 85
                                    ? const Color(0xFFFFFBEB)
                                    : const Color(0xFFFEF2F2),
                            borderRadius: const BorderRadius.circular(8),
                          ),
                          child: Text('${row.productivityIndex}%',
                              style: TextStyle(
                                fontWeight: FontWeight.w700,
                                fontFeatures: const [
                                  FontFeature.tabularFigures()
                        ...
                        DataCell(_numberCell(
                            '${row.absenteeismRate.toStringAsFixed(1)}%',
                            row.absenteeismRate > 6.0
                                ? const Color(0xFFEF4444)
                                : row.absenteeismRate > 4.0
                                    ? const Color(0xFFF59E0B)
                                    : const Color(0xFF22C55E))),
                        DataCell(_numberCell(
                            '${row.skillGap}',
                            row.skillGap >= 3
                                ...
                                : row.skillGap >= 1
                                    ...
                                    : const Color(0xFF22C55E))),
                        DataCell(_numberCell(
                            '${row.backlogWeeks.toStringAsFixed(1)}w',
                            row.backlogWeeks > 5.0
                                ? const Color(0xFFEF4444)
                                : row.backlogWeeks > 3.5
                                    ? const Color(0xFFF59E0B)
                        ...
                        DataCell(Row(
                          ...
                          children: [
                            IconButton(
                                icon: const Icon(Icons.edit_outlined,
                                    size: 18, color: Color(0xFF3B82F6)),
                                onPressed: () => _showCapacityHealthDialog(
                                    context,
                                    editIndex: idx),
                                splashRadius: 18,
                                tooltip: 'Edit'),
                            IconButton(
                                icon: const Icon(Icons.delete_outline,
                                    size: 18, color: Color(0xFFEF4444)),
                                onPressed: () => _deleteCapacityHealthRow(idx),
                                splashRadius: 18,
                                tooltip: 'Delete'),
                            IconButton(
                              onPressed: () {
  ...
  Widget _buildShiftCoverageCard() {
    final totalRequired =
        _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.requiredHeadcount);
    final totalActual =
        _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.actualHeadcount);
    final avgCoverage = _shiftCoverageRows.isNotEmpty
        ? _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.coveragePercent) /
            _shiftCoverageRows.length
        : 0.0;
    final totalGap = _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.gap);
    final totalAbsences =
        _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.absenceCount);
    final totalAgency =
        _shiftCoverageRows.fold<int>(0, (sum, r) => sum + r.agencyStaff);
    final nonCompliantCount = _shiftCoverageRows
        .where((r) => r.complianceStatus == 'Non-Compliant')
        .length;
    final understaffedCount =
        _shiftCoverageRows.where((r) => r.status == 'Understaffed').length;
    return _panel(
      title: 'Shift Coverage',
      subtitle:
          'Shift scheduling headcount compliance, coverage gaps, absence tracking, agency/contractor fill, regulatory compliance, and rotation planning with risk flagging.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Summary bar
                  ...
                  child: DataTable(
                    ...
                    rows: _shiftCoverageRows.asMap().entries.map((entry) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(_numberCell('${row.requiredHeadcount}',
                            const Color(0xFF1E293B))),
                        DataCell(_numberCell(
                            '${row.actualHeadcount}',
                            row.actualHeadcount >= row.requiredHeadcount
                                ? const Color(0xFF22C55E)
                                : const Color(0xFFEF4444))),
                        DataCell(Row(
                          mainAxisSize: MainAxisSize.min,
                        ...
                        DataCell(Text(row.shiftPattern,
                            style: const TextStyle(fontSize: 12))),
                        DataCell(_numberCell(
                            '${row.overtimeHrs}h',
                            row.overtimeHrs > 15
                        ...
                        DataCell(Text(row.nextRotation,
                            style: const TextStyle(
                        ...
                        DataCell(Row(
                          ...
                          children: [
                            IconButton(
                                icon: const Icon(Icons.edit_outlined,
                                    size: 18, color: Color(0xFF3B82F6)),
                                onPressed: () => _showShiftCoverageDialog(
                                    context,
                                    editIndex: idx),
                                splashRadius: 18,
                                tooltip: 'Edit'),
                            IconButton(
                                icon: const Icon(Icons.delete_outline,
                                    size: 18, color: Color(0xFFEF4444)),
                                onPressed: () => _deleteShiftCoverageRow(idx),
                                splashRadius: 18,
                                tooltip: 'Delete'),
                            IconButton(
                              onPressed: () {
  ...
  Widget _buildStatusChip(String status) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          color: bg, borderRadius: const BorderRadius.circular(10)),
      child: Text(status,
          style:
  ...
  Future<void> _showDistributionDialog(BuildContext context,
      {int? editIndex}) async {
    final isEdit = editIndex != null;
    final existing = isEdit ? _distributionRows[editIndex] : null;
    final categoryCtrl = TextEditingController(text: existing?.category ?? '');
    final openItemsCtrl = TextEditingController(
        text: existing != null ? '${existing.openItems}' : '0');
    final criticalCtrl = TextEditingController(
        text: existing != null ? '${existing.critical}' : '0');
    final highCtrl = TextEditingController(
        text: existing != null ? '${existing.high}' : '0');
    final mediumCtrl = TextEditingController(
        text: existing != null ? '${existing.medium}' : '0');
    final lowCtrl =
        TextEditingController(text: existing != null ? '${existing.low}' : '0');
    final closedCtrl = TextEditingController(
        text: existing != null ? '${existing.closed}' : '0');
    final ownerCtrl = TextEditingController(text: existing?.owner ?? '');
    final lastUpdatedCtrl =
        TextEditingController(text: existing?.lastUpdated ?? 'Just now');
    ...
    try {
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        controller: openItemsCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Open Items',
                            border: const OutlineInputBorder()),
                      )),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Status',
                          border: const OutlineInputBorder()),
                      items: ['Active', 'Under Review', 'Monitoring', 'At Risk']
                          .map(
              ...
              FilledButton(
                onPressed: () {
                  final row = _DistributionRow(
                    ...
                    status: status,
                    lastUpdated: lastUpdatedCtrl.text.trim().isNotEmpty
                        ? lastUpdatedCtrl.text.trim()
                        : 'Just now',
                  );
                  setState(() {
                    if (isEdit) {
      ...
      closedCtrl.dispose();
      ownerCtrl.dispose();
      lastUpdatedCtrl.dispose();
    }
  }

  Future<void> _showVelocityDialog(BuildContext context,
      {int? editIndex}) async {
    final isEdit = editIndex != null;
    final existing = isEdit ? _velocityRows[editIndex] : null;
    final workstreamCtrl =
        TextEditingController(text: existing?.workstream ?? '');
    final openItemsCtrl = TextEditingController(
        text: existing != null ? '${existing.openItems}' : '0');
    final closedThisSprintCtrl = TextEditingController(
        text: existing != null ? '${existing.closedThisSprint}' : '0');
    final velocityCtrl = TextEditingController(
        text: existing != null ? '${existing.velocity}' : '50');
    final throughputCtrl = TextEditingController(
        text: existing != null ? '${existing.throughput}' : '0.0');
    final deltaCtrl = TextEditingController(text: existing?.delta ?? '+0.0%');
    final avgCycleTimeCtrl = TextEditingController(
        text: existing != null ? '${existing.avgCycleTime}' : '0.0');
    final periodCtrl =
        TextEditingController(text: existing?.period ?? 'Sprint 41-42');
    final ownerCtrl = TextEditingController(text: existing?.owner ?? '');
    ...
    try {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      Expanded(
                          child: VoiceTextField(
                        controller: closedThisSprintCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Closed Sprint',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        decoration: const InputDecoration(
                            labelText: 'Delta (e.g. +8.2%)',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: avgCycleTimeCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Avg Cycle Time (days)',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Status',
                          border: const OutlineInputBorder()),
                      items: ['On Track', 'Improving', 'Stable', 'At Risk']
                          .map(
                              (s) => DropdownMenuItem(value: s, child: Text(s)))
                          .toList(),
                      onChanged: (v) =>
                          setDialogState(() => status = v ?? 'On Track'),
                    ),
                  ],
                ),
              ),
              ...
              FilledButton(
                onPressed: () {
                  final row = _ActionVelocityRow(
                    ...
                    openItems: int.tryParse(openItemsCtrl.text) ?? 0,
                    closedThisSprint:
                        int.tryParse(closedThisSprintCtrl.text) ?? 0,
      ...
      workstreamCtrl.dispose();
      openItemsCtrl.dispose();
      closedThisSprintCtrl.dispose();
      velocityCtrl.dispose();
      throughputCtrl.dispose();
      deltaCtrl.dispose();
      avgCycleTimeCtrl.dispose();
      periodCtrl.dispose();
      ownerCtrl.dispose();
    }
  }
  ...
  void _deleteDistributionRow(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete "${_distributionRows[index].category}"? This action cannot be undone.'),
        shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20)),
        actions: [
          TextButton(
  ...
  void _deleteVelocityRow(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete "${_velocityRows[index].workstream}"? This action cannot be undone.'),
        shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20)),
        actions: [
          TextButton(
  ...
  Future<void> _showCapacityHealthDialog(BuildContext context,
      {int? editIndex}) async {
    final isEdit = editIndex != null;
    final existing = isEdit ? _capacityHealthRows[editIndex] : null;
    final teamCtrl = TextEditingController(text: existing?.team ?? '');
    final plannedFteCtrl = TextEditingController(
        text: existing != null ? '${existing.plannedFte}' : '0.0');
    final allocatedFteCtrl = TextEditingController(
        text: existing != null ? '${existing.allocatedFte}' : '0.0');
    final availableFteCtrl = TextEditingController(
        text: existing != null ? '${existing.availableFte}' : '0.0');
    final utilizationCtrl = TextEditingController(
        text: existing != null ? '${existing.utilization}' : '0');
    final overallocatedCtrl = TextEditingController(
        text: existing != null ? '${existing.overallocated}' : '0.0');
    final fteVarianceCtrl = TextEditingController(
        text: existing != null ? '${existing.fteVariance}' : '0.0');
    final burnRateCtrl = TextEditingController(
        text: existing != null ? '${existing.burnRate}' : '0');
    final productivityIndexCtrl = TextEditingController(
        text: existing != null ? '${existing.productivityIndex}' : '100');
    final overtimeHrsCtrl = TextEditingController(
        text: existing != null ? '${existing.overtimeHrs}' : '0');
    final absenteeismRateCtrl = TextEditingController(
        text: existing != null ? '${existing.absenteeismRate}' : '0.0');
    final skillGapCtrl = TextEditingController(
        text: existing != null ? '${existing.skillGap}' : '0');
    final backlogWeeksCtrl = TextEditingController(
        text: existing != null ? '${existing.backlogWeeks}' : '0.0');
    final costVarianceCtrl = TextEditingController(
        text: existing != null ? '${existing.costVariance}' : '0');
    final ownerCtrl = TextEditingController(text: existing?.owner ?? '');
    final lastUpdatedCtrl =
        TextEditingController(text: existing?.lastUpdated ?? 'Just now');
    ...
    try {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                          child: VoiceTextField(
                        controller: plannedFteCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Planned FTE',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: allocatedFteCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Allocated FTE',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: availableFteCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Available FTE',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    Row(children: [
                      ...
                      Expanded(
                          child: VoiceTextField(
                        controller: overallocatedCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Overallocated FTE',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: fteVarianceCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'FTE Variance',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    Row(children: [
                      ...
                      Expanded(
                          child: VoiceTextField(
                        controller: productivityIndexCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Productivity Index %',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    Row(children: [
                      ...
                      Expanded(
                          child: VoiceTextField(
                        controller: absenteeismRateCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                    ...
                    Row(children: [
                      Expanded(
                          child: VoiceTextField(
                        controller: skillGapCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Skill Gap Count',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: backlogWeeksCtrl,
                        keyboardType: const TextInputType.numberWithOptions(
                            decimal: true),
                        decoration: const InputDecoration(
                            labelText: 'Backlog Weeks',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: costVarianceCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Cost Variance %',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: ['High', 'Medium', 'Low']
                              ...
                              .toList(),
                          onChanged: (v) =>
                              setDialogState(() => riskLevel = v ?? 'Medium'),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
      ...
      teamCtrl.dispose();
      plannedFteCtrl.dispose();
      allocatedFteCtrl.dispose();
      availableFteCtrl.dispose();
      utilizationCtrl.dispose();
      overallocatedCtrl.dispose();
      fteVarianceCtrl.dispose();
      burnRateCtrl.dispose();
      productivityIndexCtrl.dispose();
      overtimeHrsCtrl.dispose();
      absenteeismRateCtrl.dispose();
      skillGapCtrl.dispose();
      backlogWeeksCtrl.dispose();
      costVarianceCtrl.dispose();
      ownerCtrl.dispose();
      lastUpdatedCtrl.dispose();
    }
  }
  ...
  void _deleteCapacityHealthRow(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete "${_capacityHealthRows[index].team}"? This action cannot be undone.'),
        shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20)),
        actions: [
          TextButton(
  ...
  Future<void> _showShiftCoverageDialog(BuildContext context,
      {int? editIndex}) async {
    final isEdit = editIndex != null;
    final existing = isEdit ? _shiftCoverageRows[editIndex] : null;
    final shiftCtrl = TextEditingController(text: existing?.shift ?? '');
    final requiredHeadcountCtrl = TextEditingController(
        text: existing != null ? '${existing.requiredHeadcount}' : '0');
    final actualHeadcountCtrl = TextEditingController(
        text: existing != null ? '${existing.actualHeadcount}' : '0');
    final coveragePercentCtrl = TextEditingController(
        text: existing != null ? '${existing.coveragePercent}' : '0');
    final gapCtrl =
        TextEditingController(text: existing != null ? '${existing.gap}' : '0');
    final shiftPatternCtrl =
        TextEditingController(text: existing?.shiftPattern ?? '');
    final overtimeHrsCtrl = TextEditingController(
        text: existing != null ? '${existing.overtimeHrs}' : '0');
    final contractorFillCtrl = TextEditingController(
        text: existing != null ? '${existing.contractorFill}' : '0');
    final agencyStaffCtrl = TextEditingController(
        text: existing != null ? '${existing.agencyStaff}' : '0');
    final absenceCountCtrl = TextEditingController(
        text: existing != null ? '${existing.absenceCount}' : '0');
    final nextRotationCtrl =
        TextEditingController(text: existing?.nextRotation ?? '');
    final supervisorCtrl =
        TextEditingController(text: existing?.supervisor ?? '');
    final lastUpdatedCtrl =
        TextEditingController(text: existing?.lastUpdated ?? 'Just now');
    ...
    try {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                          child: VoiceTextField(
                        controller: requiredHeadcountCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Required Headcount',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: actualHeadcountCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Actual Headcount',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: coveragePercentCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Coverage %',
                            border: const OutlineInputBorder()),
                    ...
                    Row(children: [
                      ...
                      Expanded(
                          child: VoiceTextField(
                        controller: contractorFillCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Contractor Fill',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: agencyStaffCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Agency Staff',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                    Row(children: [
                      Expanded(
                          child: VoiceTextField(
                        controller: absenceCountCtrl,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                            labelText: 'Absence Count',
                            border: const OutlineInputBorder()),
                      )),
                      const SizedBox(width: 10),
                      Expanded(
                          child: VoiceTextField(
                        controller: nextRotationCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Next Rotation',
                            border: const OutlineInputBorder()),
                      )),
                    ]),
                    const SizedBox(height: 14),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(
                              ...
                              border: const OutlineInputBorder()),
                          items: ['Compliant', 'Conditional', 'Non-Compliant']
                              .map((s) =>
                                  DropdownMenuItem(value: s, child: Text(s)))
                              .toList(),
                          onChanged: (v) => setDialogState(
                              () => complianceStatus = v ?? 'Compliant'),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: ['None', 'Low', 'Medium', 'High', 'Critical']
                              ...
                              .toList(),
                          onChanged: (v) =>
                              setDialogState(() => riskFlag = v ?? 'None'),
                        ),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(
                              labelText: 'Status',
                              border: const OutlineInputBorder()),
                          items: ['Full', 'Covered', 'Partial', 'Understaffed']
                              .map((s) =>
                                  DropdownMenuItem(value: s, child: Text(s)))
                              .toList(),
                          onChanged: (v) =>
                              setDialogState(() => status = v ?? 'Covered'),
                        ),
                      ),
                    ]),
                  ],
              ...
              FilledButton(
                onPressed: () {
                  final row = _ShiftCoverageRow(
                    ...
                    requiredHeadcount:
                        int.tryParse(requiredHeadcountCtrl.text) ?? 0,
                    actualHeadcount:
                        int.tryParse(actualHeadcountCtrl.text) ?? 0,
                    coveragePercent:
                        int.tryParse(coveragePercentCtrl.text) ?? 0,
      ...
      shiftCtrl.dispose();
      requiredHeadcountCtrl.dispose();
      actualHeadcountCtrl.dispose();
      coveragePercentCtrl.dispose();
      gapCtrl.dispose();
      shiftPatternCtrl.dispose();
      overtimeHrsCtrl.dispose();
      contractorFillCtrl.dispose();
      agencyStaffCtrl.dispose();
      absenceCountCtrl.dispose();
      nextRotationCtrl.dispose();
  ...
  void _deleteShiftCoverageRow(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete "${_shiftCoverageRows[index].shift}"? This action cannot be undone.'),
        shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20)),
        actions: [
          TextButton(
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_punchlist_actions_notes'] ??
                'No data recorded.'),
      ],
    );
  }
