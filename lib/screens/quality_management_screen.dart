            ...
            children: [
              SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    PlanningPhaseHeader(
                        title: 'Quality Management', onExportPdf: _exportPdf),
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_quality_management_notes'] ??
                'No data recorded.'),
      ],
    );
  }
                ...
                InkWell(
                  ...
                  child: Row(
                    children: [
                      ...
                      OutlinedButton.icon(
                        onPressed:
                            _isGeneratingInsights ? null : _regenerateInsights,
                        icon: const Icon(Icons.refresh, size: 14),
...
class _TrackInExecutionBannerState extends State<_TrackInExecutionBanner> {
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text('Tracked in Execution, but calendar sync failed: $e'),
              behavior: SnackBarBehavior.floating,
            ),
          );
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isTracked ? const Color(0xFFECFDF5) : const Color(0xFFFFF8E1),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isTracked ? const Color(0xFF10B981) : const Color(0xFFFFC812),
        ),
      ),
      child: Row(
        children: [
          Icon(
            ...
            color:
                isTracked ? const Color(0xFF10B981) : const Color(0xFFFFC812),
            size: 22,
          ),
          const SizedBox(width: 12),
          Expanded(
                ...
                OutlinedButton.icon(
                  onPressed: () async {
                    final rows = await showCsvImportDialog(context,
                        tableTitle: 'Quality Standards',
                        columns: [
                          const CsvColumnSpec(
                              key: 'name',
                              label: 'Standard Name',
                              sampleValue: 'ISO 9001'),
                          const CsvColumnSpec(
                              key: 'source',
                              label: 'Source',
                              sampleValue: 'ISO'),
      ...
      table: DataTable(
        ...
        rows: [
          for (int i = 0; i < standards.length; i++)
            DataRow(
              cells: [
                DataCell(Text(standards[i].name,
                    style: const TextStyle(fontSize: 13))),
                DataCell(Text(standards[i].source,
                    style: const TextStyle(fontSize: 13))),
                DataCell(Text(standards[i].category,
                    style: const TextStyle(fontSize: 13))),
                DataCell(Text(standards[i].applicability,
                    style: const TextStyle(fontSize: 13))),
                DataCell(Text(
                    standards[i].effectiveDate.isEmpty
                        ? '—'
                        : standards[i].effectiveDate,
                    style: const TextStyle(fontSize: 13))),
                DataCell(Text(
                    standards[i].reviewDate.isEmpty
                        ? '—'
                        : standards[i].reviewDate,
                    style: const TextStyle(fontSize: 13))),
                DataCell(
                  Row(
                    mainAxisSize: MainAxisSize.min,
          ...
          for (int i = 0; i < objectives.length; i++)
            DataRow(
              cells: [
                DataCell(SizedBox(
                    width: 220,
                    child: Text(objectives[i].title,
                        style: const TextStyle(fontSize: 13)))),
                DataCell(SizedBox(
                    width: 140,
                    child: Text(objectives[i].successMetric,
                        style: const TextStyle(fontSize: 13)))),
        ...
        rows: [
          for (final control in controls)
            DataRow(cells: [
              ...
              DataCell(SizedBox(
                  width: 220,
                  child: Text(control.method,
                      style: const TextStyle(fontSize: 13)))),
              DataCell(SizedBox(
                  width: 140,
                  child: Text(control.tools,
                      style: const TextStyle(fontSize: 13)))),
              DataCell(Text(control.frequency,
                  style: const TextStyle(fontSize: 13))),
              DataCell(SizedBox(
                  width: 140,
                  child: Text(control.owner,
                      style: const TextStyle(fontSize: 13)))),
              DataCell(SizedBox(
                  width: 180,
                  child: Text(control.standardsReference,
                      style: const TextStyle(fontSize: 13)))),
              DataCell(Row(
                mainAxisSize: MainAxisSize.min,
                children: [
...
class _QualityObjectiveDialogState extends State<_QualityObjectiveDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final dialogWidth =
        screenWidth >= 1000 ? 760.0 : (screenWidth - 32).clamp(0.0, 760.0);
    return AlertDialog(
      title: Text(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            _metricCard(
                'Open / In Progress', '$openCount', const Color(0xFFF59E0B)),
            _metricCard('Overdue', '$overdueCount', const Color(0xFFDC2626)),
            _metricCard('Verified', '$verifiedCount', const Color(0xFF10B981)),
          ],
        ),
        const SizedBox(height: 24),
        _PrimaryCard(
