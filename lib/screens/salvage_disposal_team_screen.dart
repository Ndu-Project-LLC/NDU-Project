...
class _SalvageDisposalTeamScreenState extends State<SalvageDisposalTeamScreen> {
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['salvage_disposal_team_screen'] ??
                'No data recorded.'),
      ],
    );
  }
      ...
      if (timelineSnap.docs.isEmpty) {
        final defaultTimeline = [
          (
            ...
            'Complete physical inventory and condition assessment of all assets',
            'Planning',
            'Completed',
          ...
          (
            'Vendor Bidding Opens',
            'Issue RFPs and open bidding for disposal vendors',
            'Execution',
            'Completed',
            'Procurement Lead',
            '2026-03-16',
          ...
          (
            'Auction Date',
            'Conduct public auction for high-value assets',
            'Execution',
            'In Progress',
            'Auction Coordinator',
            '2026-03-21',
          ...
          (
            'Final Disposal Report',
            'Compile final disposal documentation and reconciliation report',
            'Review',
            'Not Started',
            'Compliance Officer',
            '2026-03-29',
          ...
          (
            'Project Closure',
            'Formal project sign-off and archive disposal records',
            'Closure',
            'Not Started',
            'Project Manager',
            '2026-04-06',
    ...
    try {
      ...
      if (data != null && data['rows'] != null) {
        final rows = (data['rows'] as List)
            .map((e) =>
                _ComplianceRegulationRow.fromMap(e as Map<String, dynamic>))
            .toList();
        if (rows.isNotEmpty) {
          setState(() {
            _complianceRows
  ...
  Widget _buildAssetInventoryContent(bool isNarrow) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 24),
        if (isNarrow) _buildInventoryTable() else _buildInventoryTable(),
      ],
    );
  }
                ...
                onPressed: () async {
                  final rows = await showCsvImportDialog(context,
                      ...
                      columns: [
                        ...
                        CsvColumnSpec(
                            key: 'name',
                            label: 'Asset Name',
                            sampleValue: 'Server Rack'),
                        CsvColumnSpec(
                            key: 'category',
                            label: 'Category',
                            sampleValue: 'Physical Infrastructure'),
                        CsvColumnSpec(
                            key: 'condition',
                            label: 'Condition',
                            sampleValue: 'Good',
                        ...
                        CsvColumnSpec(
                            key: 'location',
                            label: 'Location',
                            sampleValue: 'Main Site'),
                        CsvColumnSpec(
                            key: 'status',
                            label: 'Status',
                            sampleValue: 'Ready',
                            allowedValues: ['Ready', 'Pending', 'Flagged']),
                        CsvColumnSpec(
                            key: 'estimatedValue',
                            label: 'Est. Value',
                            sampleValue: '5200'),
                  ...
                  if (mounted)
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text('${rows.length} items imported'),
  ...
  Widget _buildInventoryTableContent() {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<SalvageInventoryItemModel>>(
  ...
  Future<void> _showEditInventoryDialog(
      BuildContext context, SalvageInventoryItemModel item) async {
    final projectId = _getProjectId();
...
''';
  ...
  Future<void> _showTeamMemberDialog(
      BuildContext context, SalvageTeamMemberModel? member) async {
    final projectId = _getProjectId();
    if (projectId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    try {
                  ...
                  try {
                    if (isEdit) {
                      await SalvageService.updateTeamMember(
                        ...
                        itemsHandled:
                            int.tryParse(itemsHandledController.text.trim()) ??
                                0,
                      );
                    } else {
                      await SalvageService.createTeamMember(
      ...
      itemsHandledController.dispose();
    }
  }
  ...
  Future<void> _showDisposalDialog(
      BuildContext context, SalvageDisposalItemModel? item) async {
    final projectId = _getProjectId();
    if (projectId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    var selectedDisposalMethod = item?.disposalMethod.isNotEmpty == true
        ? item!.disposalMethod
        : 'Auction';
    ...
    try {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: LaunchModalDropdown<String>(
                        ...
                        onChanged: (v) {
                          if (v != null)
                            setDialogState(() => selectedDisposalMethod = v);
                        },
                      ),
                    ),
                  ],
      ...
      locationController.dispose();
      valueController.dispose();
      disposalCostController.dispose();
  ...
  Widget _buildQueueBoard() {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<SalvageDisposalItemModel>>(
        ...
        children: [
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: () async {
                  final rows = await showCsvImportDialog(context,
                      ...
                      columns: [
                        ...
                        CsvColumnSpec(
                            key: 'role',
                            label: 'Role',
                            sampleValue: 'Product Owner'),
                        CsvColumnSpec(
                            key: 'focus',
                            label: 'Focus Area',
                            sampleValue: 'Operations Support'),
                        CsvColumnSpec(
                            key: 'workload',
                            label: 'Workload %',
                            sampleValue: '48'),
                  ...
                  if (mounted)
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text('${rows.length} members imported'),
                        backgroundColor: Colors.green));
                },
                icon: const Icon(Icons.upload_file_outlined, size: 16),
                label: const Text('Import CSV'),
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              return RepaintBoundary(
                child: StreamBuilder<List<SalvageTeamMemberModel>>(
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                        'Track assets through the disposal workflow per ITAD / ISO 14001 / NIST SP 800-88 standards',
                        style:
                            TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  ],
                ),
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            ...
                            columns: const [
                              ...
                              DataColumn(
                                  label: Text('Est. Value'), numeric: true),
                              DataColumn(
                                  label: Text('Disp. Cost'), numeric: true),
                            ...
                            rows: items.isEmpty
                                ...
                                : items.map((item) {
                                    final priorityColor =
                                        _priorityColorFor(item.priority);
                                    return DataRow(
                                      cells: [
                                        ...
                                        DataCell(
                                            _buildCategoryChip(item.category)),
                                        DataCell(_buildConditionChip(
                                            item.condition)),
                                        DataCell(_buildDisposalMethodChip(
                                            item.disposalMethod)),
                                        ...
                                        DataCell(Text(
                                            ...
                                            style: TextStyle(
                                                fontSize: 11,
                                                color: item.location.isNotEmpty
  ...
  void _showDisposalItemDetailDialog(
      BuildContext context, SalvageDisposalItemModel item) {
    showDialog(
      context: context,
      barrierDismissible: true,
        ...
        body: Column(
          ...
          children: [
            ...
            _detailRow('Condition',
                item.condition.isNotEmpty ? item.condition : 'Not specified'),
            _detailRow('Location',
                item.location.isNotEmpty ? item.location : 'Not specified'),
            _detailRow(
                'Disposal Method',
                item.disposalMethod.isNotEmpty
                    ? item.disposalMethod
                    : 'Not specified'),
            ...
            _detailRow('Assigned To',
                item.assignedTo.isNotEmpty ? item.assignedTo : 'Unassigned'),
            _detailRow('Target Date',
                item.targetDate.isNotEmpty ? item.targetDate : 'Not set'),
  ...
  Widget _buildCompliancePanel() {
    ...
    final compliantCount =
        _complianceRows.where((r) => r.complianceStatus == 'Compliant').length;
    final nonCompliantCount = _complianceRows
        .where((r) => r.complianceStatus == 'Non-Compliant')
        .length;
    final renewalDueCount = _complianceRows
        .where((r) => r.complianceStatus == 'Renewal Due')
        .length;
    final pendingCount =
        _complianceRows.where((r) => r.complianceStatus == 'Pending').length;
    final criticalRiskCount = _complianceRows
        .where((r) => r.riskLevel == 'Critical' || r.riskLevel == 'High')
        .length;
    final avgScore = totalRegs > 0
        ? _complianceRows.fold<int>(0, (sum, r) => sum + r.complianceScore) /
            totalRegs
        : 0.0;
    final totalFindings =
        _complianceRows.fold<int>(0, (sum, r) => sum + r.findings);
    final totalCorrective =
        _complianceRows.fold<int>(0, (sum, r) => sum + r.correctiveActions);
    final expiringSoon = _complianceRows
        .where((r) => r.daysToExpiry >= 0 && r.daysToExpiry <= 30)
        .length;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
              'Environmental, safety, health, legal, and financial regulatory compliance tracking with audit scheduling, risk flagging, and corrective action management.',
                  ...
                  child: ConstrainedBox(
                    ...
                    child: DataTable(
                      ...
                      rows: _complianceRows.asMap().entries.map((entry) {
                        ...
                        return DataRow(cells: [
                          DataCell(Text(row.regulation,
                              style: const TextStyle(
                                  fontWeight: FontWeight.w700, fontSize: 11))),
                          DataCell(_buildComplianceCategoryChip(row.category)),
                          DataCell(
                              _buildComplianceStatusChip(row.complianceStatus)),
                          DataCell(Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              SizedBox(
                              ...
                              Text('${row.complianceScore}',
                                  style: TextStyle(
                                    fontWeight: FontWeight.w700,
                                    fontSize: 11,
                                    color: row.complianceScore >= 90
                                        ? const Color(0xFF22C55E)
                                        : row.complianceScore >= 70
                                            ? const Color(0xFF2563EB)
                                            : row.complianceScore >= 50
                          ...
                          DataCell(Text(row.lastAuditDate,
                              style: const TextStyle(
                                  fontSize: 10, color: Color(0xFF64748B)))),
                          DataCell(Text(row.nextAuditDue,
                              style: const TextStyle(
                                  fontSize: 10, color: Color(0xFF64748B)))),
                          DataCell(Container(
                          ...
                          DataCell(Text(row.responsibleParty,
                              style: const TextStyle(fontSize: 10))),
                          DataCell(_buildComplianceRiskChip(row.riskLevel)),
                          DataCell(Text('${row.findings}',
                              style: TextStyle(
                                fontWeight: FontWeight.w700,
                                color: row.findings > 5
                                    ? const Color(0xFFEF4444)
                                    : row.findings > 0
                                        ? const Color(0xFFF59E0B)
                                        : const Color(0xFF22C55E),
                              ))),
                          DataCell(Text('${row.correctiveActions}',
                              style: TextStyle(
                                fontWeight: FontWeight.w700,
                                color: row.correctiveActions > 3
                                    ? const Color(0xFFEF4444)
                          ...
                          DataCell(Row(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () =>
                                    _showComplianceRegulationDialog(context,
                                        editIndex: idx),
                                padding: EdgeInsets.zero,
                                constraints: const BoxConstraints(
  ...
  Future<void> _showComplianceRegulationDialog(BuildContext context,
      {int? editIndex}) async {
    final isEdit = editIndex != null;
    final existing = isEdit ? _complianceRows[editIndex] : null;
    final regulationCtrl =
        TextEditingController(text: existing?.regulation ?? '');
    final lastAuditCtrl =
        TextEditingController(text: existing?.lastAuditDate ?? '');
    final nextAuditCtrl =
        TextEditingController(text: existing?.nextAuditDue ?? '');
    final daysToExpiryCtrl = TextEditingController(
        text: existing != null ? '${existing.daysToExpiry}' : '90');
    final responsibleCtrl =
        TextEditingController(text: existing?.responsibleParty ?? '');
    final findingsCtrl = TextEditingController(
        text: existing != null ? '${existing.findings}' : '0');
    final correctiveCtrl = TextEditingController(
        text: existing != null ? '${existing.correctiveActions}' : '0');
    final scoreCtrl = TextEditingController(
        text: existing != null ? '${existing.complianceScore}' : '100');
    ...
    try {
                  ...
                  children: [
                    Expanded(
                      child: LaunchModalDropdown<String>(
                        ...
                        items: const ['Critical', 'High', 'Medium', 'Low'],
                        onChanged: (v) =>
                            setDialogState(() => riskLevel = v ?? 'Low'),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
      ...
      lastAuditCtrl.dispose();
      nextAuditCtrl.dispose();
      daysToExpiryCtrl.dispose();
      ...
      child: Column(
        ...
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                        'Milestone tracking with phases, ownership, and progress per project management standards',
                        style:
                            TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  ],
                ),
          ...
          if (projectId == null)
            ...
            RepaintBoundary(
              child: StreamBuilder<List<SalvageTimelineItemModel>>(
                            ...
                            rows: items.map((item) {
                              return DataRow(cells: [
                                ...
                                DataCell(Text(
                                    ...
                                    style: TextStyle(
                                        fontSize: 11,
                                        color: item.owner.isNotEmpty
                                            ? const Color(0xFF1E293B)
                                            : const Color(0xFF94A3B8)))),
                                DataCell(Text(
                                    item.startDate.isNotEmpty
                                        ? item.startDate
                                        : '-',
                                    style: const TextStyle(
                                        fontSize: 11,
                                  ...
                                  children: [
                                    IconButton(
                                      icon: const Icon(Icons.edit_outlined,
                                          size: 14),
                                      onPressed: () => _showEditTimelineDialog(
                                          context, item),
                                      padding: EdgeInsets.zero,
                                      constraints: const BoxConstraints(
                                          minWidth: 28, minHeight: 28),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                    'No timeline milestones have been created yet. Click "Add Milestone" to create your first disposal milestone, or the default milestones will be shown below.',
  ...
  void _showEditTimelineDialog(
      BuildContext context, SalvageTimelineItemModel item) {
    _showTimelineDialog(context, item);
  }

  Future<void> _showTimelineDialog(
      BuildContext context, SalvageTimelineItemModel? item) async {
    final projectId = _getProjectId();
    if (projectId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    final dependenciesController =
        TextEditingController(text: item?.dependencies ?? '');
                ...
                Column(
                  ...
                  children: [
                    ...
                    Slider(
                      ...
                      onChanged: (v) =>
                          setDialogState(() => progressValue = v.round()),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
  ...
  void _showDeleteTimelineDialog(
      BuildContext context, SalvageTimelineItemModel item) {
    final projectId = _getProjectId();
    if (projectId == null) return;
...
class _ComplianceRegulationRow {
  ...
  static _ComplianceRegulationRow fromMap(Map<String, dynamic> map) =>
      _ComplianceRegulationRow(
        ...
        complianceScore: (map['complianceScore'] is int)
            ? map['complianceScore'] as int
            : int.tryParse(map['complianceScore'].toString()) ?? 0,
        lastAuditDate: map['lastAuditDate']?.toString() ?? '',
        nextAuditDue: map['nextAuditDue']?.toString() ?? '',
        daysToExpiry: (map['daysToExpiry'] is int)
            ? map['daysToExpiry'] as int
            : int.tryParse(map['daysToExpiry'].toString()) ?? 0,
        responsibleParty: map['responsibleParty']?.toString() ?? '',
        riskLevel: map['riskLevel']?.toString() ?? 'Low',
        findings: (map['findings'] is int)
            ? map['findings'] as int
            : int.tryParse(map['findings'].toString()) ?? 0,
        correctiveActions: (map['correctiveActions'] is int)
            ? map['correctiveActions'] as int
            : int.tryParse(map['correctiveActions'].toString()) ?? 0,
