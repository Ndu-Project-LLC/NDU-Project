...
class _SummarizeAccountRisksScreenState
  ...
  Widget _buildPerformanceInsights() {
    ...
    if (costData != null) {
      for (final sol in costData.solutionCosts) {
        for (final row in sol.costRows) {
          final v =
              double.tryParse(row.cost.replaceAll(RegExp(r'[^0-9.]'), '')) ?? 0;
          budgetTotal += v;
          budgetUsed += v * 0.94;
        }
      }
    ...
    final scopeItems =
        projectData.withinScope.length + projectData.outOfScope.length;
        ...
        (projectData.coreStakeholdersData?.solutionStakeholderData ?? [])
            .fold<int>(
                ...
                (s,
                        e) =>
                    s +
                    (e.internalStakeholders.isNotEmpty ? 1 : 0) +
                    (e.externalStakeholders.isNotEmpty ? 1 : 0));
    final stakeholderScore = stakeholderCount == 0
        ? 0.6
    ...
    final overallScore =
        radarAxes.fold<double>(0, (s, a) => s + a.value) / radarAxes.length;
        ...
        LaunchInsightsHeader(
          ...
          kpiTiles: [
            ...
            LaunchKpiTile(
              ...
              delta: budgetTotal == 0 ? 'no cost data' : 'of \$$budgetTotal',
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'highlight',
            label: 'Highlight',
            sampleValue: 'On-time delivery'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'All milestones delivered on schedule'),
      ],
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            dropdownItems: ['Open', 'Mitigated', 'Closed'])
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'risk', label: 'Risk', sampleValue: 'Budget overrun'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Actual spend exceeded plan by 15%'),
        CsvColumnSpec(
        ...
        CsvColumnSpec(
            ...
            label: 'Status',
            sampleValue: 'Open',
            allowedValues: ['Open', 'Mitigated', 'Closed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
  ...
  Widget _buildNext90DaysPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'priority', label: 'Priority', sampleValue: 'Complete UAT'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Finish user acceptance testing'),
        CsvColumnSpec(key: 'owner', label: 'Owner', sampleValue: 'QA Lead'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Planned',
            allowedValues: ['Planned', 'In Progress', 'Complete']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
        ...
        if (mounted) {
          setState(() {
            _highlights[index] = _highlights[index].copyWith(
              ...
              details:
                  (parsed['details'] ?? _highlights[index].details).toString(),
            );
          });
          _save();
        }
        ...
        if (mounted) {
          setState(() {
            _topRisks[index] = _topRisks[index].copyWith(
              title: (parsed['title'] ?? '').toString(),
              details:
                  (parsed['details'] ?? _topRisks[index].details).toString(),
  ...
  Future<void> _regenerateNext90Row(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Next 90 Days Focus');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _next90Days[index] = _next90Days[index].copyWith(
              title: (parsed['title'] ?? '').toString(),
              details:
                  (parsed['details'] ?? _next90Days[index].details).toString(),
              owner: (parsed['owner'] ?? _next90Days[index].owner).toString(),
              status:
                  (parsed['status'] ?? _next90Days[index].status).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_highlights.isEmpty && _topRisks.isEmpty && _next90Days.isEmpty) {
    ...
    try {
      ...
      for (final s in cp.scopeTracking) {
        ...
        if (status == 'verified' || status == 'completed' || status == 'done') {
          if (s.deliverable.isNotEmpty &&
              !highlightExisting.contains(s.deliverable)) {
            newHighlights.add(LaunchHighlightItem(
              title: s.deliverable,
      ...
      for (final d in cp.deliverableRows) {
        ...
        if (status != 'completed' && status != 'done' && status != 'verified') {
          final title = d['title']?.toString() ?? '';
          if (title.isNotEmpty &&
              !next90Existing.contains('Complete: $title')) {
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          ...
          'next_90_days':
              'Immediate follow-up priorities with "title", "details", "owner", "status"',
        },
        itemsPerSection: 3,
      );
    } catch (e) {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final filename =
          'project_summary_${projectName.replaceAll(' ', '_')}_$stamp.pdf';
