...
class _TransitionToProdTeamScreenState
  ...
  Widget _buildTeamRosterPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'startDate', label: 'Start Date', sampleValue: '2025-01-15'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Active',
    ...
    return LaunchDataTable(
      ...
      columns: const [
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: LaunchHandoverItem.categories),
        LaunchColumn(
            label: 'Item',
            flexible: true,
            fieldType: LaunchFieldType.text,
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'category',
            label: 'Category',
            sampleValue: 'Documentation',
            allowedValues: LaunchHandoverItem.categories),
        CsvColumnSpec(
            key: 'item', label: 'Item', sampleValue: 'Handover runbook'),
  ...
  Widget _buildSignOffsPanel() {
    return LaunchDataTable(
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            key: 'role', label: 'Role', sampleValue: 'Operations Director'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
        ...
        CsvColumnSpec(
            key: 'notes', label: 'Notes', sampleValue: 'Waiting for review'),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
  ...
  Future<void> _autoPopulateFromPriorPhases() async {
    ...
    try {
      ...
      if (_teamRoster.isEmpty && cp.staffing.isNotEmpty) {
        ...
        final newMembers =
            cp.staffing.where((m) => !existing.contains(m.name)).toList();
  ...
  Future<void> _populateFromAi() async {
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          'team_roster':
              'Production team members with "name", "role", "contact", "release_status"',
          'handover_checklist':
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final filename =
          'transition_to_prod_${projectName.replaceAll(' ', '_')}_$stamp.pdf';
  ...
  Future<void> _regenerateTeamRosterRow(int index) async {
    ...
    try {
      ...
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Production Team Roster');
  ...
  Future<void> _regenerateHandoverRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Handover Checklist');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _handoverChecklist[index] = _handoverChecklist[index].copyWith(
              category:
                  (parsed['category'] ?? _handoverChecklist[index].category)
                      .toString(),
              item: (parsed['item'] ?? '').toString(),
              owner: (parsed['owner'] ?? _handoverChecklist[index].owner)
                  .toString(),
              status: (parsed['status'] ?? _handoverChecklist[index].status)
                  .toString(),
            );
          });
          _scheduleSave();
  ...
  Future<void> _regenerateSignOffRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Ops & Client Sign-Offs');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
    ...
    return LaunchInsightsHeader(
      ...
      kpiTiles: [
        ...
        LaunchKpiTile(
          ...
          value:
              '${projectData.keyMilestones.where((m) => m.comments.toLowerCase().contains('complete') || m.comments.toLowerCase().contains('done')).length}',
