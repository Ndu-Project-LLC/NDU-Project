...
class _UpdateOpsMaintenancePlansScreenState
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['update_ops_maintenance_plans_screen'] ??
                'No data recorded.'),
      ],
    );
  }
  ...
  Widget _buildPlanRegisterBody(String? projectId) {
    ...
    return RepaintBoundary(
      child: StreamBuilder<List<OpsPlanItem>>(
          ...
          if (_coverage.isEmpty)
            ...
            ListView.builder(
              ...
              itemBuilder: (context, i) => RepaintBoundary(
                key: ValueKey('coverage_row_$i'),
                child: _buildCoverageRow(_coverage[i]),
              ),
            ),
        ...
        children: [
          if (_signals.isEmpty)
            ...
            ListView.builder(
              ...
              itemBuilder: (context, i) => RepaintBoundary(
                key: ValueKey('signal_row_$i'),
                child: _buildSignalRow(_signals[i]),
              ),
            ),
          const SizedBox(height: 12),
        ...
        children: [
          if (_maintenanceWindows.isEmpty)
            ...
            ListView.builder(
              ...
              itemBuilder: (context, i) => RepaintBoundary(
                key: ValueKey('maintenance_row_$i'),
                child: _buildMaintenanceRow(_maintenanceWindows[i]),
              ),
            ),
        ],
      ),
  ...
  Widget _buildMaintenanceRow(_MaintenanceWindowItem item) {
    ...
    final statusColor =
        getStatusColor(item.status.isEmpty ? 'Scheduled' : item.status);
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
