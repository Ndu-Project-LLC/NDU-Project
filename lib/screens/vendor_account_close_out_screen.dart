...
class _VendorAccountCloseOutScreenState
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            dropdownItems: ['Active', 'Closing', 'Closed']),
        LaunchColumn(
            label: 'Outstanding',
            flexible: true,
            fieldType: LaunchFieldType.text,
      ...
      csvColumns: const [
        ...
        CsvColumnSpec(
            ...
            label: 'Status',
            sampleValue: 'Active',
            allowedValues: ['Active', 'Closing', 'Closed']),
        CsvColumnSpec(
            key: 'outstanding',
            label: 'Outstanding',
            sampleValue: 'Pending invoice'),
        CsvColumnSpec(
            key: 'notes', label: 'Notes', sampleValue: 'Final payment pending'),
      ],
    ...
    return LaunchDataTable(
      ...
      columns: const [
        ...
        LaunchColumn(
            ...
            width: 120,
            fieldType: LaunchFieldType.dropdown,
            dropdownItems: ['Pending', 'Revoked', 'Confirmed'])
      ],
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'system', label: 'System', sampleValue: 'AWS Console'),
        CsvColumnSpec(key: 'vendor', label: 'Vendor', sampleValue: 'Acme Corp'),
        CsvColumnSpec(
            key: 'accessLevel', label: 'Access Level', sampleValue: 'Admin'),
        CsvColumnSpec(
            key: 'revokedDate',
            label: 'Revoked Date',
            sampleValue: '2025-01-20'),
        CsvColumnSpec(
            key: 'status',
            label: 'Status',
            sampleValue: 'Pending',
            allowedValues: ['Pending', 'Revoked', 'Confirmed']),
      ],
      onCsvImport: (rows) async {
        for (final row in rows) {
          setState(() {
    ...
    return LaunchDataTable(
      ...
      csvColumns: const [
        CsvColumnSpec(
            key: 'task',
            label: 'Task',
            sampleValue: 'Revoke all vendor access'),
        CsvColumnSpec(
            key: 'details',
            label: 'Details',
            sampleValue: 'Remove VPN, email, and system accounts'),
  ...
  Future<void> _autoPopulateFromPriorPhases() async {
    ...
    try {
      ...
      if (_vendors.isEmpty && cp.vendors.isNotEmpty) {
        final existing = _vendors.map((v) => v.vendorName).toSet();
        final newVendors =
            cp.vendors.where((v) => !existing.contains(v.vendorName)).toList();
    ...
    try {
      result = await LaunchPhaseAiSeed.generateEntries(
        ...
        sections: const {
          ...
          'closure_checklist':
              'Closure verification tasks with "title", "details", "status"',
        },
        itemsPerSection: 3,
      );
    } catch (e) {
        ...
        if (mounted) {
          setState(() {
            _vendors[index] = _vendors[index].copyWith(
              ...
              contractRef:
                  (parsed['contract_ref'] ?? _vendors[index].contractRef)
                      .toString(),
              outstandingItems:
                  (parsed['outstanding'] ?? _vendors[index].outstandingItems)
                      .toString(),
              accountStatus: (parsed['status'] ?? _vendors[index].accountStatus)
                  .toString(),
  ...
  Future<void> _regenerateAccessRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Access Revocation');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _accessItems[index] = _accessItems[index].copyWith(
              ...
              vendor:
                  (parsed['vendor'] ?? _accessItems[index].vendor).toString(),
              accessLevel:
                  (parsed['access_level'] ?? _accessItems[index].accessLevel)
                      .toString(),
              status:
                  (parsed['status'] ?? _accessItems[index].status).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _regenerateObligationRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Outstanding Obligations');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _obligations[index] = _obligations[index].copyWith(
              ...
              details:
                  (parsed['details'] ?? _obligations[index].details).toString(),
              owner: (parsed['owner'] ?? _obligations[index].owner).toString(),
              status:
                  (parsed['status'] ?? _obligations[index].status).toString(),
            );
          });
          _save();
        }
  ...
  Future<void> _regenerateClosureChecklistRow(int index) async {
    ...
    try {
      final projectData = ProjectDataHelper.getData(context);
      final ctx = ProjectDataHelper.buildProjectContextScan(projectData,
          sectionLabel: 'Account Closure Checklist');
      final openai = OpenAiServiceSecure();
      final result = await openai.generateCompletion(
        ...
        if (mounted) {
          setState(() {
            _closureChecklist[index] = _closureChecklist[index].copyWith(
              ...
              details: (parsed['details'] ?? _closureChecklist[index].details)
                  .toString(),
              owner: (parsed['owner'] ?? _closureChecklist[index].owner)
                  .toString(),
              status: (parsed['status'] ?? _closureChecklist[index].status)
                  .toString(),
            );
          });
          _save();
          ...
          build: (_) => [
            ...
            if (_obligations.isEmpty)
              ...
              pw.TableHelper.fromTextArray(
                ...
                cellPadding:
                    const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 5),
                headers: const ['Obligation', 'Details', 'Owner', 'Status'],
                data: _obligations
                    .map((o) => [
                          _pc(o.title),
                          _pc(o.details),
