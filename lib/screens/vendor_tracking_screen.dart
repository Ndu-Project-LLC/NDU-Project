...
class _VendorTrackingScreenState extends State<VendorTrackingScreen> {
          ...
          builder: (context, snapshot) {
            ...
            final rows = <_KpiRow>[
              _KpiRow(
                  ...
                  value: onTimeAvg,
                  target: 0.90,
                  trend: onTimeAvg >= 0.90 ? 'On target' : 'Below target',
                  owner: 'Vendor Manager',
              ...
              _KpiRow(
                  ...
                  value: incidentAvg,
                  target: 0.85,
                  trend: incidentAvg >= 0.85 ? 'On target' : 'Below target',
                  owner: 'Operations Lead',
                  source: 'Auto-computed'),
              _KpiRow(
                  id: 'auto_quality',
                  ...
                  value: qualityAvg,
                  target: 0.80,
                  trend: qualityAvg >= 0.80 ? 'On target' : 'Below target',
                  owner: 'QA Lead',
                  source: 'Auto-computed'),
              _KpiRow(
                  id: 'auto_cost',
                  metric: 'Cost adherence',
                  value: costAvg,
                  target: 0.85,
                  trend: costAvg >= 0.85 ? 'On target' : 'Below target',
                  owner: 'Finance Lead',
                  source: 'Auto-computed'),
              ..._customKpiRows,
            ];
              ...
              children: [
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      Expanded(
                          flex: 3,
                          child: Text('KPI Metric', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1,
                          child: Text('Actual', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1,
                          child: Text('Target', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1, child: Text('Gap', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Trend', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Owner', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1, child: Text('', style: _perfHeaderStyle)),
                    ],
                  ),
                ),
                ...
                if (_customKpiRows.isNotEmpty)
                  Padding(
                    ...
                    child: Text(
                        '${_customKpiRows.length} custom metric${_customKpiRows.length != 1 ? 's' : ''}',
                        style: const TextStyle(
        ...
        builder: (ctx) => AlertDialog(
          title: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(isEdit ? 'Edit KPI Metric' : 'Add KPI Metric',
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                final val =
                    (int.tryParse(valueCtl.text.trim()) ?? 0).clamp(0, 100) /
                        100.0;
                final tgt =
                    (int.tryParse(targetCtl.text.trim()) ?? 85).clamp(0, 100) /
                        100.0;
                final row = _KpiRow(
                  id: existing?.id ??
                  ...
                  value: val,
                  target: tgt,
                  trend: trendCtl.text.trim().isNotEmpty
                      ? trendCtl.text.trim()
                      : (val >= tgt ? 'On target' : 'Below target'),
                ...
                setState(() {
                  if (isEdit) {
                    final idx =
                        _customKpiRows.indexWhere((r) => r.id == row.id);
        ...
        child: StreamBuilder<List<VendorModel>>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasData && snapshot.data!.isNotEmpty) {
              ...
              final watchCount =
                  vendors.where((v) => v.status == 'Watch').length;
              ...
              children: [
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      Expanded(
                          flex: 3,
                          child: Text('Signal', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1,
                          child: Text('Severity', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Category', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Owner', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1,
                          child: Text('Status', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1, child: Text('', style: _perfHeaderStyle)),
                    ],
                  ),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Expanded(
                                    flex: 2,
                                    child: Text(sig.category,
                                        style: const TextStyle(
                                            fontSize: 11,
                                            color: Color(0xFF6B7280)))),
                                Expanded(
                                    flex: 2,
                                    child: Text(sig.owner,
                                        style: const TextStyle(
                                            fontSize: 11,
                                            color: Color(0xFF6B7280)))),
                                Expanded(
                ...
                if (_customSignalRows.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(top: 6),
                    child: Text(
                        '${_customSignalRows.length} custom signal${_customSignalRows.length != 1 ? 's' : ''}',
                        style: const TextStyle(
                            fontSize: 11, color: Color(0xFF9CA3AF))),
                  ),
              ],
      ...
      await showDialog(
        ...
        builder: (ctx) => StatefulBuilder(
          builder: (ctx, setDState) => AlertDialog(
            title: Row(children: [
              Icon(isEdit ? Icons.edit_outlined : Icons.warning_amber_rounded,
                  size: 20, color: const Color(0xFF4154F1)),
              const SizedBox(width: 8),
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        items: ['Critical', 'High', 'Medium', 'Low']
                            ...
                            .toList(),
                        onChanged: (v) {
                          if (v != null) setDState(() => severity = v);
                        },
                      )),
                      const SizedBox(width: 12),
                      Expanded(
                        ...
                        decoration: const InputDecoration(
                            ...
                            isDense: true,
                            border: OutlineInputBorder()),
                        items: ['Open', 'Monitoring', 'Mitigated', 'Closed']
                            .map((s) =>
                                DropdownMenuItem(value: s, child: Text(s)))
                            .toList(),
                        onChanged: (v) {
              ...
              FilledButton(
                onPressed: () {
                  ...
                  setState(() {
                    if (isEdit) {
                      final idx =
                          _customSignalRows.indexWhere((r) => r.id == row.id);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    const Text(
                        'Add touchpoints, reviews, and remediation tasks.',
                        style:
          ...
          : Column(
              children: [
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      Expanded(
                          flex: 3,
                          child: Text('Action Item', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 1,
                          child: Text('Priority', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Due Date', style: _perfHeaderStyle)),
                      Expanded(
                          flex: 2,
                          child: Text('Owner', style: _perfHeaderStyle)),
                      Expanded(
                            ...
                            Expanded(
                              ...
                              child: Row(
                                  ...
                                  children: [
                                    InkWell(
                                        onTap: () =>
                                            _showActionDialog(existing: act),
                                        child: const Icon(Icons.edit_outlined,
                                            size: 14,
                                            color: Color(0xFF6B7280))),
                                    const SizedBox(width: 4),
                                    InkWell(
                                        onTap: () => _removeAction(act.id),
                ...
                Padding(
                  padding: const EdgeInsets.only(top: 6),
                  child: Text(
                      '${_actionRows.length} action${_actionRows.length != 1 ? 's' : ''}',
                      style: const TextStyle(
                          fontSize: 11, color: Color(0xFF9CA3AF))),
                ),
              ],
          ...
          builder: (ctx, setDState) => AlertDialog(
            title: Row(children: [
              Icon(isEdit ? Icons.edit_outlined : Icons.add_circle_outline,
                  size: 20, color: const Color(0xFF4154F1)),
              const SizedBox(width: 8),
              Text(isEdit ? 'Edit Action Item' : 'Add Action Item',
                  style: const TextStyle(fontSize: 16)),
            ]),
            content: SizedBox(
              width: 480,
                  ...
                  Row(children: [
                    Expanded(
                      ...
                      items: ['Critical', 'High', 'Medium', 'Low']
                          ...
                          .toList(),
                      onChanged: (v) {
                        if (v != null) setDState(() => priority = v);
                      },
                    )),
                    const SizedBox(width: 12),
                    Expanded(
  ...
  Future<void> _showVendorDialog(
    ...
    try {
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  ...
                  final quality =
                      double.tryParse(qualityController.text) ?? 0.0;
      ...
      slaPerformanceController.dispose();
      leadTimeController.dispose();
      requiredDeliverablesController.dispose();
  ...
  Future<void> _exportPdf() async {
    ...
    await PdfExportHelper.exportScreenPdf(
      ...
      sections: [
        ...
        PdfSection.text(
            'Notes',
            projectData.planningNotes['planning_vendor_tracking_notes'] ??
                'No data recorded.'),
      ],
    );
  }
